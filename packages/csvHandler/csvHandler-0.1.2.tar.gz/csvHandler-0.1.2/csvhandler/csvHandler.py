...
def combine(file_path, added_path='', deleted_path='', changed_original_path='', changed_new_path=''):
    ...
    cursor.execute('create index id_idx on ' + TABLE_NAME + '(' + ID_COLUMN_STR + ');')
    cursor.execute('create index record_type_idx on ' + TABLE_NAME + '(' + RECORD_TYPE_STR + ');')
    connection.commit()
    connection.close()
...
class CsvHandler():
    def __init__(self, file_path):
        ...
        connection = sqlite3.connect(file_path)
        columns = pd.read_sql('select * from ' + TABLE_NAME + ' limit 0', con=connection).columns.tolist() # Get columns
    ...
    def get_page(self, page_number, page_size, orderby='', is_asc=True, filter_exp='', projection=[], exclude_columns=[]):
        ...
        where_clause = self.__odata_filter_to_where(filter_exp) if filter_exp != '' else ''
        self.metadata['rows']['filtered'] = None

        if where_clause != '':
            rows_number = self.__calculate_rows(where_clause, con=connection)      
            self.metadata['rows']['filtered'] = rows_number

        data_frame = self.__get_page(page_number, page_size, orderby, is_asc, where_clause, projection, exclude_columns, con=connection)
        connection.close()
        return self.__page_format(data_frame)
    
    def __get_page(self, page_number, page_size, orderby, is_asc, where_clause, projection, exclude_columns, con):
    ...
    def __calculate_rows(self, where_clause, con):
        data_frame = pd.read_sql('select ' + RECORD_TYPE_STR + ',count(*) from ' + TABLE_NAME + where_clause + ' group by ' + RECORD_TYPE_STR, con=con)
    ...
    def __select(self, projection, exclude_columns):
        columns = projection if len(projection) > 0 else list(self.metadata['columns'])
        #Exclude columns
        ...
        columns_str = '*' if len(projection) == 0 and len(exclude_columns) == 0 else ','.join(columns)
    ...
    def __odata_filter_to_where(self, query):
        comparison_operators_map = {' eq ': ' = ', ' ne ': ' != ', ' gt ': ' > ', ' lt ': ' < ', ' ge ': ' >= ', ' le ': ' <= '}
        logical_operators_map = {' not ': ' NOT ', ' and ': ' AND ', ' or ': ' OR '}
