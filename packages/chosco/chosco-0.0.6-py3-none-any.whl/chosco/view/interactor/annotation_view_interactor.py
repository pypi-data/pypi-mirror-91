...
class AnnotationViewInteractor(ViewInteractor):
    ...
    def _create_labeler_box(self, output):
        ...
        if self.optional_widgets:
            ...
            total_widgets += self.optional_widgets

        for widget in total_widgets:
            self.default_widget_values[widget.description] = widget.value
    ...
    def execute(self, output: Output):
        ...
        self.labeler_box = self._create_labeler_box(output)

        main_layout = Layout(flex_flow="column", width="100%")
        main_box = Box(children=[self.labeler_box, output], layout=main_layout)
    ...
    def update_header_box(
        ...
        self.progress.value = status.number_of_annotations
        self.item_id.value = f"Waiting for annotation: {image_id}"
        with self.output:
            display(image)

    def clear_labeler_box(self):
        if self.labeler_box:
            for tabs in self.labeler_box.children:
                if not hasattr(tabs, "children"):
                    continue
                for tab in tabs.children:
                    for widget in tab.children:
                        widget.value = self.default_widget_values.get(
                            widget.description
                        )

    def update_labeler_box(self, metadata: dict):
        if self.labeler_box:
            for tabs in self.labeler_box.children:
                if not hasattr(tabs, "children"):
                    continue
                for tab in tabs.children:
                    for widget in tab.children:
                        widget.value = metadata.get(widget.description)
