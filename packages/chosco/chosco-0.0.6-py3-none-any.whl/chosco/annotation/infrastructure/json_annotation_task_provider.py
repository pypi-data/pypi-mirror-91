...
class JsonAnnotationTaskProvider(AnnotationTaskProvider):
    ...
    def from_filename(filename: str):
        with open(filename, "r") as infile:
            ...
            return JsonAnnotationTaskProvider(
                kdict.get("done"), kdict.get("pending"), kdict.get("session_id")
            )
    ...
    def __init__(self, done: List[str], pending: List[str], session_id: str):
        self.done = done
    ...
    def get_total_annotations(self):
        return self.get_number_of_annotations() + self.get_number_pending_annotations()
    ...
    def next(self) -> str:
        if len(self.pending) > 0:
            self.current_task_id = self.pending[-1]
        else:
            self.current_task_id = None
    ...
    def previous(self) -> str:
        if len(self.done) > 0:
            self.current_task_id = self.done[-1]
        else:
            self.current_task_id = None
        return self.current_task_id
