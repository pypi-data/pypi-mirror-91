...
class CLIRecord(Record):
    ...
    def __init__(self, pipe=False, title=None, **kwargs):
        super().__init__(title=title, **kwargs)

        print_item_created(self, title=title, pipe=pipe)
    ...
    def get_file(self, filepath, force, file_name=None, file_id=None):
        ...
        if file_id is not None:
            ...
            list_file_ids.append(super().get_file_id(file_name))
        ...
        for name_iter, id_iter in zip(list_file_names, list_file_ids):
            ...
            if response.status_code == 200:
                ...
                click.echo(
                    f"Something went wrong when trying to download file '{file_name}' "
    ...
    def delete_file(self, file):
        ...
        response = super().delete_file(file)
        if response.status_code != 204:
            raise_request_error(response)
        else:
    ...
    def delete_files(self, i_am_sure):
        ...
        if response.status_code == 200:
            ...
            for page in range(1, total_pages + 1):
                for results in payload["items"]:
                    response_delete = super().delete_file(file_id_or_name=results["id"])
