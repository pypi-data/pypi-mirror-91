...
def print_item_created(item, title=None, pipe=False):
...
def item_delete(item, i_am_sure):
    ...
    if response.status_code == 204:
        click.echo(f"Deleting of was successful.")
    else:
...
def item_add_record_link(item, record_to):
    ...
    if response.status_code == 201:
        ...
        click.echo(
            f"Link from {item} to {record_to} already exsists. " "Nothing to do."
        )
    else:
...
def item_add_collection_link(item, collection):
    ...
    if response.status_code == 201:
        ...
        click.echo(
            f"Link from {item} to {collection} already exsists. " "Nothing to do."
        )
    else:
