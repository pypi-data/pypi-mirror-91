...
class line:
    ...
    def _add(self, value, item):
        put(bytes([self.id, int(self.translation[item]), 1]) + int(value).to_bytes(4, byteorder='little'))

    def _set(self, value, item):
        put(bytes([self.id, int(self.translation[item]), 0]) + str(value).encode("utf-8"))
...
class buffer:
    ...
    def append(self, message):
        put(bytes([self.id, 1]) + message.encode("utf-8"))
