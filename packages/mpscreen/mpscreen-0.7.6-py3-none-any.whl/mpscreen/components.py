...
def put(fifo,  msg: bytes):
    to_write = create_msg(msg)
    os.write(fifo, to_write)
    #os.close(fifo)


class line:
    def __init__(self,  id, attributes, translation):
        self.id = id
        self.attributes = attributes
        self.translation = translation
        self.fifo = os.open(IPC_FIFO_NAME, os.O_WRONLY | os.O_APPEND)

    def __setattr__(self, key, value):
        if key in ["q", "id", "attributes","translation", "fifo"]:
    ...
    def _add(self, value, item):
        put(self.fifo, bytes([self.id, int(self.translation[item]), 1]) + int(value).to_bytes(4, byteorder='little'))

    def _set(self, value, item):
        put(self.fifo, bytes([self.id, int(self.translation[item]), 0]) + str(value).encode("utf-8"))
...
class buffer:
    ...
    def append(self, message):
        put(self.fifo, bytes([self.id, 1]) + message.encode("utf-8"))

    def clear(self):
        put(self.fifo, bytes([self.id, 0]))
