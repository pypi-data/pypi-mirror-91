...
def _get_trace_provider():
    ...
    processor = AmlContextSpanProcessor(
        UserFacingSpanProcessor(
            ExporterSpanProcessor(
                RunHistoryExporter(logger=logger),
                logger=logger)))

    _trace_provider = DefaultTraceProvider(AmlTracer([processor]))
    return _trace_provider
