...
{
    return PyUnicode_FromFormat("n_bins: %d, min: %f, max: %f, width: %f", self->n_bins, self->min, self->max, self->width);
}
...
static PyMethodDef Bins_methods[] = {
    ...
    {"put", (PyCFunction)Bins_put, METH_FASTCALL,
     "Put item into bin for value"
    },
    {"get", (PyCFunction)Bins_get, METH_FASTCALL,
     "Get item and bin number for value"
    },
    {"items", (PyCFunction)Bins_items, METH_FASTCALL,
         "Get items in bins"
    },
    {"props", (PyCFunction)Bins_items, METH_NOARGS,
         "Get Bins properties as string"
    },
...
static PyModuleDef binstoremodule = {
    PyModuleDef_HEAD_INIT,
    .m_name = "binstore",
    .m_doc = "module to implement Bins class for bin-based object storage",
    .m_size = -1,
};
...
PyInit_binstore(void)
{
    ...
    m = PyModule_Create(&binstoremodule);
    if (m == NULL)
        return NULL;
