...
class MCDA:
    ...
    def topsis(self, normalization_method=RootSumSquared_):  
    ...
    def __waspas(self, lambda_):
        if self.normalization_method in [Max_, MinMax_] or all(n == 1 for n in self.signals):
             w_df = pd.DataFrame(self.df_weighted.iloc[:, 1:])       
        else:
            raise ValueError("WASPAS does not run with chosen normalization method for minimization criteria. Try Max_ or MinMax_ instead.")
    ...
    def __wsm(self):
        if self.normalization_method in [Max_, MinMax_] or all(n == 1 for n in self.signals):
            ...
            raise ValueError("WSM does not run with chosen normalization method for minimization criteria. Try Max_ or MinMax_ instead.")
    ...
    def __wpm(self):
        if self.normalization_method in [Max_, MinMax_] or all(n == 1 for n in self.signals):
            ...
            raise ValueError("WPM does not run with chosen normalization method for minimization criteria. Try Max_ or MinMax_ instead.")
