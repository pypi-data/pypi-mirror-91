...
class SimpleTable:
    ...
    def import_csv(self, csvfile, header=None):
        if header:
            self.__header = header
            self.__num_of_cols = len(self.__header)
        with open(csvfile, "r") as f:
            reader = csv.reader(f)
            data.extend([row for row in reaqder])
    ...
    def __splitrow(self,row) :
        if self.__maxwidth < 20 :
            ...
            return zip_longest(*[self.__splitstring(str(c),self.__maxwidth) for c in row],fillvalue="")
    ...
    def __repr__(self):
        ...
        for ix, w in enumerate(twidth):
            twidth[ix] = w - wcswidth(str(self.__header[ix])) + len(str(self.__header[ix]))
...
def main():
    ...
    if args.pivot:
        ...
        t = SimpleTable(header=oheader, data=data, cols=args.dumpcols,maxwidth=args.maxwidth)
        if args.format == "json" :
            print(t.get_json())
