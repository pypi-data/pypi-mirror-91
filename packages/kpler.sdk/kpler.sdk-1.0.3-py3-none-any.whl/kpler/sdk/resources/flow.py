...
class Flow(KplerClient):
    """
    The ``Flow`` endpoint returns the aggregated flows for a point of interest (installation/zone) on a daily, weekly,
    ...
    def get(
        ...
        Args:
             flow_direction: Optional[List[Enum]] ``FlowDirection``
             split: Optional[List[Enum]] ``FlowSplit``
             granularity: Optional[List[Enum]] ``FlowPeriod``
             ...
             unit: Optional[List[Enum]] ``FlowMeasurementUnit``
        ...
        Examples:
            ...
            >>>
            from datetime import date
            from kpler.sdk import FlowDirection, FlowSplit, FlowPeriod, FlowMeasurementUnit
            flows_client = Flow(config)
            flows_client.get(
                flow_direction=[FlowDirection.Export],
                split=[FlowSplit.OriginCountries],
                granularity=[FlowPeriod.Daily],
                unit=[FlowMeasurementUnit.T],
