...
class Price(KplerClient):

    """
    The ``Price`` endpoint allows you to extract the latest price information in $/MMBtu for various indexes and price
    ...
    def get_columns(self) -> DataFrame:
        """
        This endpoint returns a recent and updated list of all columns available for the endpoint price.

        Examples:
            >>>
            from kpler.sdk.resources.price import Price
            price_client = Price(config)
            price_client.get_columns()
    ...
    def get(
        ...
        Args:
            ...
            indexes: Optional[List[Enum]] ``PriceIndexes``
        ...
        Examples:
            ...
            from datetime import date
            from kpler.sdk.resources.price import Price
            from kpler.sdk import PriceIndexes
            price_client = Price(config)
            price_client.get(
                start_date=date(2020,9,1),
                end_date=date(2020,10,1),
                size=10,
                indexes=[PriceIndexes.HenryHub, PriceIndexes.TTF]
            )
