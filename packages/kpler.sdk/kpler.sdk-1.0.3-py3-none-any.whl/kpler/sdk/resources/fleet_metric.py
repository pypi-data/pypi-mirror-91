...
class FleetMetric(KplerClient):

    """
    The ``FleetMetric`` endpoint provides you with aggregated fleet data per day or week, either the floating storage or all
    the loaded vessels.
    """
    ...
    def get(
        ...
        Args:
            ...
            metric: Optional[Enum] ``FleetMetricAlgo``
            period: Optional[Enum] ``FleetMetricPeriod``
            ...
            unit: Optional[Enum] ``MeasurementUnit``
            split: Optional[Enum] ``FleetMetricSplit``

        Examples:

            >>>
            from kpler.sdk.resources.fleet_metric import FleetMetric
            from kpler.sdk import FleetMetricAlgo, FleetMetricSplit
            fleet_metrics_client = FleetMetric(config)
            fleet_metrics_client.get(
                metric=FleetMetricAlgo.LoadedVessels,
                zones=["China"],
                period=FlowPeriod.Daily,
                split=FleetMetricSplit.CurrentSubregions
            )
