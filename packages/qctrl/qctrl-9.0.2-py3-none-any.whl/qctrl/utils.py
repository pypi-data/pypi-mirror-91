...
def separate_list(source: list, func) -> tuple:
    """Returns two lists. First list will only include items which have a non-
    zero result when calling fn; second list will be every other item in the
    source.
    ...
    -------
    tuple
        two list one is matched the other one is not matched.
    """
    matched = []
    unmatched = []
...
def get_resource_schema(schema: dict, resource_name: str) -> dict:
    """Given the API schema, returns the schema for the given resource (if
    exists).
    ...
    -------
    dict
        nested value in the schema.
    """
    return get_nested_value(schema, "components.schemas.{}".format(resource_name), {})


def normalize_encoded_field_name(
    name: str = None, many: list = None, dict_keys: dict = None
) -> Any:
    """A function to normalize the field name based on the passing data type
    (str, list or dict)

    Parameters
    ----------
    name: str
         field name. (Default value = None)
    many: list
         list field. (Default value = None)
    dict_keys: dict
         dict field. (Default value = None)
    ...
    """
    pattern = re.compile("_encoded$")

    if name:
        return pattern.sub("", name)

    if many:
        return [normalize_encoded_field_name(x) for x in many]

    if dict_keys:
        result_dict_keys = {}
        for key, value in dict_keys.items():
            new_key = normalize_encoded_field_name(key)
            result_dict_keys.update({new_key: value})
...
def check_client_version(func):
    ...
    the QCTRL Python Package defined by the API.
    """
    ...
    def raise_exception(exc):
        ...
        raise VersionError(
            "Current version of QCTRL Python Package is not compatible with API. "
