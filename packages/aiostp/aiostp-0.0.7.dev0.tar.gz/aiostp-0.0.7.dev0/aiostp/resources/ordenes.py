...
class Orden(Resource, OrdenSTPMex):
    ...
    # tiene que ser del mismo día hábil
    @classmethod
    async def _consulta_clave_rastreo_enviada(
        cls,
        claveRastreo: str,
    ...
    @classmethod
    async def consulta_clave_rastreo(
        cls,
        claveRastreo: str,
        ...
        """
        # for now we are only able to check outgoing
        institucionOperante = int(institucionOperante)
        if institucionOperante == STP_BANK_CODE:  # enviada
            return await cls._consulta_clave_rastreo_enviada(
                claveRastreo, institucionOperante, fechaOperacion
            )
        else:
            # I still don't know how to handle incoming transactions
            # because the previous method used the consulta()
            # function, that has a limit for 2 requests per day.
            return None
