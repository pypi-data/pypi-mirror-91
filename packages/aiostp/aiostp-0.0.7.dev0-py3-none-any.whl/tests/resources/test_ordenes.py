...
import aiostp
from aiostp.resources.ordenes import STP_BANK_CODE
...
async def test_consulta_orden_enviada_clave_rastreo():
    ...
    enviada = await aiostp.Orden.consulta_clave_rastreo(
        'CR1610641277', STP_BANK_CODE, fechaOperacion=fecha
    )
...
@pytest.mark.asyncio
@pytest.mark.vcr
async def test_consulta_orden_recibida_clave_rastreo():
    enviada = await aiostp.Orden.consulta_clave_rastreo(
        'CR1610641277', '23323'
    )

    assert enviada is None
