...
def getFederate():
    ...
    return {
            "Name": "server name",
            "Address": f'{str(random.randint(1,255))}.{str(random.randint(1,255))}.{str(random.randint(1,255))}.{str(random.randint(1,255))}',
            "Port": str(random.randint(1, 65535)),
            "FallBack": "fallback server name",
            "Status": str(random.choice(["Enabled", "Disabled"])),
            "ReconnectInterval": str(random.randint(0, 100)),
            "MaxRetries": str(random.choice(['Unlimited', str(random.randint(1, 30))])),
            "LastError": random.choice(["Timeout", "None"])
            }
