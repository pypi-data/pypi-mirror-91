...
class Server(ServerConfig):
    ...
    def __init__(self, config_path, *args, **kwargs):
        ...
        if self.port is None:
            self.port = self.server.bind_to_random_port('tcp://*', min_port=6001, max_port=6004, max_tries=100)
        else:
        ...
        self.logger.info(f"Worker ids:  {'; '.join([str(x.id) for x in self.workers])}")
    ...
    def add_worker(self, config_path, python_path):
        ...
        p = subprocess.Popen("script2.py 1", shell=True)

        os.system(f"{python_path} script2.py 1")
