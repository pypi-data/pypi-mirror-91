...
def verify_interface_hold_time(device, interface, expected_hold_time='0', position='up', max_time=60, check_interval=10):
    ...
    while timeout.iterate():
        ...
        if out.q.get_values('{}-hold-time'.format(position), 0) == expected_hold_time:
            return True
...
def verify_interface_data(device, interface, expected_physical_description=None, 
                          expected_logical_description=None, expected_crc_errors=None,
                          extensive=False, max_time=60, check_interval=10):
    ...
    while timeout.iterate():
        ...
        if expected_logical_description:
            logical_description = out.q.contains('logical-interface').get_values('description',0)

        if expected_physical_description:
            physical_description = out.q.contains('physical-interface').not_contains('logical-interface').get_values('description',0)
        ...
        if expected_crc_errors !=None and str(crc_error) == str(expected_crc_errors):
            timeout.sleep()
            continue
        return True
    ...
    while timeout.iterate():
        ...
        if local_addresses:
            ...
            if expected_interface:
                ...
                local_addresses_no_mask = [i.split('/')[0] for i in local_addresses]
