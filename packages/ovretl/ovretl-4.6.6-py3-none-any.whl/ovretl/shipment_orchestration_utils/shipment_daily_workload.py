...
def daily_tasks_total_weight_on_shipment(shipment_id: int, tasks_df: pd.DataFrame, date: datetime.date) -> int:
...
def daily_workload_calculation(shipments_timelines_df: pd.DataFrame, tasks_df: pd.DataFrame) -> pd.DataFrame:
    ...
    tasks_df.loc[:, "updated_at"] = tasks_df["updated_at"].apply(datetime_to_date)
