...
def get_taobao_time():
    """获取苏宁时间{"sysTime2":"2021-01-12 16:41:50","sysTime1":"20210112164150"}"""
    try:
        url = 'http://10.228.84.20:21000/get_local_timestamp'
        ts = requests.get(url)
        if ts.status_code == 200:
            json_data = json.loads(ts.text)
            return {"sysTime2": timestamp_toString(json_data.get('time')),"sysTime1": f"{json_data.get('time')}"}
        else:
            json_data = {"sysTime2": datetime_toString_detail(datetime.datetime.now()),
                         "sysTime1": f"{datetime_toTimestamp(datetime.datetime.now())}"}
            return json_data
    except:
        json_data = {"sysTime2": datetime_toString_detail(datetime.datetime.now()),"sysTime1": f"{datetime_toTimestamp(datetime.datetime.now())}"}
        return json_data
