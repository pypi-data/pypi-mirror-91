...
def start(format: bool = False):
    ...
    for c in combinators:
        ...
        for i, arg in enumerate(sorted_args):
            ...
            if arg_docs:
                arg_docs = arg_docs["params"].get(arg_name, "Telegram API parameter.")
            else:
                arg_docs = "Telegram API parameter."
        ...
        if c.section == "types":
            ...
            docstring += f"\n    Constructor type of :obj:`~pyrogram.raw.base.{c.qualtype}`."
        else:
            method_docs = docs["method"].get(c.qualname, None)

            if method_docs:
                docstring += method_docs["desc"] + "\n"
            else:
                docstring += f"Telegram API method."

        docstring += f"\n\n    Details\n        - Layer: ``{layer}``\n        - ID: ``{c.id}``\n\n"
        docstring += f"    {'Parameters:' if docstring_args else 'Parameters'}\n        " + \
                     (f"\n        ".join(docstring_args) if docstring_args else "No parameters required.")

        if c.section == "functions":
            return_docs = docs["type"].get(c.qualtype, None)

            if return_docs:
                return_docs = return_docs["desc"]
            else:
                return_docs = "Telegram API base type." if c.qualtype not in CORE_TYPES else "Primitive type."

            docstring += f"\n    Returns\n        {get_docstring_arg_type(c.qualtype)} -- {return_docs}"
        else:
