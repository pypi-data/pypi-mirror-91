...
BMLX_ADDR = "http://www.mlp.bigo.inner"
...
class AlarmTpl(object):
    def generate_run_link(self, exp_run_id: str):
        return "%s/runs/%s" % (BMLX_ADDR, exp_run_id)
    ...
    def renderEmail(self, alarm: Alarm) -> MIMEMultipart:
        ...
        content = '<a href="{link}">任务链接</a><br/>pipeline名称{pipeline_name}<br/> 信息: {message} '.format(
            ...
            link=self.generate_run_link(alarm.vars["pipeline_execution"].id),
        )
