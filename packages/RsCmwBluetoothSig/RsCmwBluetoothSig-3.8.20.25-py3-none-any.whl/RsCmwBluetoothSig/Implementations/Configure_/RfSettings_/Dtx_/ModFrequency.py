...
class ModFrequency:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("modFrequency", core, parent)

	@property
	def nmode(self):
		"""nmode commands group. 1 Sub-classes, 0 commands."""
		if not hasattr(self, '_nmode'):
			from .ModFrequency_.Nmode import Nmode
	...
	def tmode(self):
		...
		if not hasattr(self, '_tmode'):
			from .ModFrequency_.Tmode import Tmode
	...
	def lowEnergy(self):
		...
		if not hasattr(self, '_lowEnergy'):
			from .ModFrequency_.LowEnergy import LowEnergy
			self._lowEnergy = LowEnergy(self._core, self._base)
		return self._lowEnergy

	def clone(self) -> 'ModFrequency':
		"""Clones the group by creating new object from it and its whole existing sub-groups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = ModFrequency(self._core, self._base.parent)
