...
class AidOverride:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("aidOverride", core, parent)

	@property
	def cte(self):
		"""cte commands group. 0 Sub-classes, 1 commands."""
		if not hasattr(self, '_cte'):
			from .AidOverride_.Cte import Cte
			self._cte = Cte(self._core, self._base)
		return self._cte

	def clone(self) -> 'AidOverride':
		"""Clones the group by creating new object from it and its whole existing sub-groups
		Also copies all the existing default Repeated Capabilities setting,
		which you can change independently without affecting the original group"""
		new_group = AidOverride(self._core, self._base.parent)
