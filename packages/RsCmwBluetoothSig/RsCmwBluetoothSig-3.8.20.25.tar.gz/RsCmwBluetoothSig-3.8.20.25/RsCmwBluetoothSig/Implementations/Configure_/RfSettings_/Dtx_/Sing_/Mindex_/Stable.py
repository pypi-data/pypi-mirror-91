...
class Stable:
	...
	def __init__(self, core: Core, parent):
		self._core = core
		self._base = CommandsGroup("stable", core, parent)

	@property
	def tmode(self):
		"""tmode commands group. 1 Sub-classes, 0 commands."""
		if not hasattr(self, '_tmode'):
			from .Stable_.Tmode import Tmode
	...
	def lowEnergy(self):
		...
		if not hasattr(self, '_lowEnergy'):
			from .Stable_.LowEnergy import LowEnergy
	...
	def clone(self) -> 'Stable':
		...
		new_group = Stable(self._core, self._base.parent)
