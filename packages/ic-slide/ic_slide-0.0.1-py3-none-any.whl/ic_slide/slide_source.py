...
def enumerate_tiles(slide_id, begin, stop, stride, size=np.array((512, 512))):
    slide = open_slide(slide_id)
    logger.info(f"Enumerate slide {slide_id}  tiles.")
    return _enumerate_slide_tiles(slide, begin, stop, stride, size)


def _enumerate_slide_tiles(slide, begin, stop, stride, size):
...
def enumerate_tiles_auto_scaling(slide_id, begin, stop, stride, size=np.array((512, 512))):
    slide = open_slide(slide_id)
    scale = _get_slide_scale(slide)
    logger.info(f"Enumerate slide {slide_id}  tiles by auto scaling.")
    return _enumerate_slide_tiles(slide, begin, stop, stride * scale, size * scale)


def enumrate_content_tiles(slide_id, stride=512, size=np.array((512, 512))):
    slide = open_slide(slide_id)
    logger.info(f"Enumerate slide {slide_id} content tiles .")
    begin = int(slide._metadata.ContentRegion['X']), int(
        slide._metadata.ContentRegion['Y'])

    stop = begin[0] + int(slide._metadata.ContentRegion["Width"]
                          ), begin[1]+int(slide._metadata.ContentRegion["Height"])
...
def enumrate_content_tiles_auto_scaling(slide_id, stride, size=np.array((512, 512))):
    slide = open_slide(slide_id)
    logger.info(f"Enumerate slide {slide_id} content tiles by auto scaling.")
    scale = _get_slide_scale(slide)
    begin = int(slide._metadata.ContentRegion['X']), int(
        slide._metadata.ContentRegion['Y'])
...
def _get_slide_scale(slide, mpp_x=0.35093510, mpp_y=0.35150376):
    slide_metadata = slide._metadata
