...
class TreeModelDebriefing(object):
    ...
    @staticmethod
    def shap_values(predict_result: dataframe.DataFrame, predict_data: dataframe.DataFrame, key, label):
        """
        ...
        if predict_result.empty():
            raise ValueError('Predict result table is empty!')
        if predict_data.empty():
            raise ValueError('Dataset test table is empty!')

        specify_column_name = 'REASON_CODE'
        if predict_result.has(specify_column_name) is False:
            raise ValueError("Predict result table does not contain the 'REASON_CODE' column!")

        reason_code = list(predict_result.collect()[specify_column_name])

        features = predict_data.columns
        try:
            features.remove(key)
        ...
        for reason_code_item in reason_code:
            ...
            for feature in features:
                shap_values_item.append(attr_dict[feature])
            shap_values.append(shap_values_item)

        return np.array(shap_values)

    @staticmethod
    def summary_plot(predict_result: dataframe.DataFrame, predict_data: dataframe.DataFrame, key, label):
        """
        ...
        if predict_data.empty():
            raise ValueError('Dataset test table is empty!')
        shap_values = TreeModelDebriefing.shap_values(predict_result, predict_data, key, label)
        shap.summary_plot(shap_values, predict_data.drop(cols=[key, label]).collect())
