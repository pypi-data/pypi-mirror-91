...
class EWAHPostgresHook(EWAHBaseHook):
    ...
    def get_connection_form_widgets() -> dict:
        ...
        return {
            "extra__ewah_postgres__ssh_conn_id": StringField(
                lazy_gettext("SSH Connection ID (optional)"),
    ...
    def pgconn(self):
        if not hasattr(self, "_pgconn"):
            if self.conn.ssh_conn_id:
                # Start SSH tunnel & connect via port forwarding
                self._ssh_hook = EWAHBaseHook.get_hook_from_conn_id(
                    conn_id=self.conn.ssh_conn_id
                )
                local_bind_address = self._ssh_hook.start_tunnel(
                    self.conn.host, self.conn.port or 5432
                )
    ...
    def close(self):
        ...
        if hasattr(self, "_pgconn"):
            # if applicable, stop SSH tunnel first
