...
def process(input_dir, save_dir, campaign_name, run_name, species_filename=None, save_csv=True):
    ...
    for filename, group in tqdm(df_grouped):
        ...
        try:
            im = skio.imread(im_filename, plugin='pil')
        except:
            print("Error opening {}".format(im_filename))
            continue
        try:
            mask = skio.imread(mask_filename, plugin='pil')
        except:
        ...
        for row in group.iterrows():
            ...
            skio.imsave(seg_im_filename, seg_im, plugin='pil')
            skio.imsave(seg_mask_filename, seg_mask, plugin='pil')
            df_cls[id-1] = cls
