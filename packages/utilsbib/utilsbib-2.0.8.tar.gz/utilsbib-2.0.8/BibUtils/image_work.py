...
class Face_cropper:
    ...
    def isolate_mouth(self, filename, mouth_only=True, preImg=None , pre_shape=None):
        try:
            if preImg is None:
                img = cv2.imread(filename)
            ...
            if len(dets) > 0:
                return PIL.Image.fromarray(crop_image), shape
            return False
        except IndexError:
            return False
...
def original_and_makeup_slices(filename,faceCropper,nb_tiles,makeup_type, makeup_color):
    ...
    if makeup_type == 0:
        mouth_original, m_o_shape = fc.isolate_mouth("", preImg=pp[0])#.convert("RGB")
        mouth_target, m_t_shape = fc.isolate_mouth("", preImg=pp[1],pre_shape=m_o_shape)
