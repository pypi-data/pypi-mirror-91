...
class eventHandler():
    ...
    def eventCatcher(self):
        ...
        while True:
            ...
            if contents["$type"] == "data.event.profileController.stateChange":
                print("[PROFILE CONTROLLER]:",contents['state'])
            elif contents["$type"] == "data.event.popup":
                print("[IMPULSE POPUP]:",contents['message'])
            elif contents["$type"] == "data.event.toast":
                print("[IMPULSE TOAST]:",contents['message'])
            else:
