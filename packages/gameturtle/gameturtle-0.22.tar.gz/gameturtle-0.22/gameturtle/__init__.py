"""
   gameturtle.py
   0.22版本的gameturtle模块有如下更新:
   1、新增randomshape，随机造型方法。
"""
...
class GameTurtle:
    sprites = {}                               # 所有角色字典
    def __init__(self,canvas,frames,pos=None,visible=True,heading=0,tag='sprite'):
        """
           canvas：海龟所在的画布
        ...
        self._cv_width = canvas.winfo_width() - 4        # 画布边框这里应该是获取
        self._cv_height = canvas.winfo_height() - 4        
    ...
    def color_collide_color(self,color1,color2,ignore_alpha=True):
        ...
        if not results:return False
        if len(color1)==3 or len(color2)==3 : ignore_alpha=True
        sprites = []                                       # 待返回的角色列表 
        for rets in results:
            array,im_a,im_b ,r ,ot = rets            
...
if __name__ == "__main__":
    ...
    title = '游戏海龟模块0.22版_by李兴球'
    root = Tk()
    root.title(title)
    
    cv = Canvas(root,width=400,height=300,bg='green')
    cv.pack()

    redim = Image.new("RGBA",(100,100),color='red')
    tom = GameTurtle(cv,redim)
