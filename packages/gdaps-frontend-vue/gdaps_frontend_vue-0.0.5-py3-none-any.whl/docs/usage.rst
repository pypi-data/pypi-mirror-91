...
To remove a plugin from the frontend, just remove the backend part (remove it from INSTALLED_APPS or uninstall it using pip) and call ``manage.py syncplugins`` afterwords. It will take care of the sync of plugins to the database, and the frontend plugins, if a frontend is installed.
