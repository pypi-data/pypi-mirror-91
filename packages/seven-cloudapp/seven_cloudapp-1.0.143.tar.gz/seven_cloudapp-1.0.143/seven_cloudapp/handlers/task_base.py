...
:LastEditTime: 2021-01-18 17:57:26
...
class TaskBaseHandler(TopBaseHandler):
    ...
    :description: 任务基础类
    """
    def check_common(self, act_info, user_info, task_info, act_id, open_id, login_token, check_task=True):
        """
        ...
        if act_info.start_date != "" and act_info.start_date != "0000-00-00 00:00:00":
            if TimeHelper.format_time_to_datetime(now_date) < TimeHelper.format_time_to_datetime(act_info.start_date):
                invoke_result['code'] = "NoAct"
                invoke_result['message'] = "活动将在" + act_info.start_date + "开启"
                return invoke_result
        if act_info.end_date != "" and act_info.end_date != "0000-00-00 00:00:00":
    ...
    def check_black(self, user_info, act_info, all_goods_order_list):
        """
    ...
    def send_lottery_value(self, log_title, user_info, update_sql, send_num, act_info, source_type, change_type, log_info="", log_desc=""):
        """
        ...
        lottery_value_log.currency_type = act_info.currency_type
        ...
        lottery_value_log.history_value = (user_info.lottery_value if act_info.currency_type == 1 else user_info.surplus_integral) - send_num
        ...
        if result:
            behavior_model.report_behavior_log(user_info.app_id, user_info.act_id, user_info.open_id, "", 'JoinUserCount', 1, act_info.act_type)
            behavior_model.report_behavior_log(user_info.app_id, user_info.act_id, user_info.open_id, "", 'TotalRewardCount', send_num, act_info.act_type)
            behavior_model.report_behavior_log(user_info.app_id, user_info.act_id, user_info.open_id, "", 'AddJoinUserCount', 1, act_info.act_type)
