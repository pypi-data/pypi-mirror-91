...
:LastEditTime: 2020-11-24 09:55:18
...
class SevenHelper:
    ...
    def get_now_day_int(self, hours=0):
        ...
        month = str(now_date.month) if now_date.month >= 10 else "0" + str(now_date.month)
        day = str(now_date.day) if now_date.day >= 10 else "0" + str(now_date.day)
        now_day = int(str(now_date.year) + month + day)
        return now_day
    ...
    def get_now_month_int(self, hours=0):
        ...
        now_date = (datetime.datetime.now() + datetime.timedelta(hours=hours))
        month = str(now_date.month) if now_date.month >= 10 else "0" + str(now_date.month)
        now_month = int(str(now_date.year) + month)
        return now_month