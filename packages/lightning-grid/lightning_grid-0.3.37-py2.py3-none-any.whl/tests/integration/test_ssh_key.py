...
class TestSSHKey:
    def test_ssh_keys_flow(self, monkeypatch):
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            key_id = invoke(
                'add', str(key_path_pub))[len('Added key with id: '):].strip()
    ...
    def test_list(self, monkeypatch):
        ...
        assert "12222" in result
        assert "AAAA GGGG ZZZZ" in result

    def test_add_ssh_key(self, monkeypatch):
        resolver = MagicMock(return_value={
            "message": "ok",
            "success": True,
            "id": "1122",
        ...
        with tempfile.TemporaryDirectory() as tmpdirname:
            ...
            key_path_pub.write_text(key_pub)
            assert invoke(
                'add', str(key_path_pub)).strip() == 'Added key with id: 1122'
        resolver.assert_called()
