...
class SummaryMNE():
    def __init__(self,df0,within=["cond0","clust0"],height=6,aspect=1):
        self.df0            = df0.copy()
        self.df1            = self.df0.copy()[["epochs0","tool0","mode0","type0","cond0","chan0","later0","front0","clust0","tmin0"]].drop_duplicates()
        self.within         = within
        self.height         = height
        self.aspect         = aspect
        self.epochs0_widget = Dropdown(options = self.df0.epochs0.unique().tolist())
        self.tool0_widget   = Dropdown()
        self.mode0_widget   = Dropdown()
        self.type0_widget   = Dropdown()
        self.tmin0_widget   = Dropdown()
        self.cond0_widget   = SelectMultiple(options=["verb","noun","noun_F","noun_M"],value=["verb","noun","noun_F","noun_M"],rows=9,description="cond0")
        self.items0         = [self.epochs0_widget,self.tool0_widget,self.mode0_widget,self.type0_widget,self.tmin0_widget,]
        self.items1         = [self.cond0_widget]
        self.box_layout     = Layout(display='flex',flex_flow='col',align_items='stretch',width='50%')
        self.box0            = VBox(children=self.items0, layout=self.box_layout)
        self.box1            = VBox(children=self.items1, layout=self.box_layout)
        self.UI              = HBox(children=[self.box0,self.box1])
