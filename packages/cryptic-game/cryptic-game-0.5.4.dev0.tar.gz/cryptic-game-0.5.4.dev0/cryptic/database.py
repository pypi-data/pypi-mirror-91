...
class DatabaseWrapper:
    ...
    def __setup_mysql(username: str, password: str, hostname: str, port: int, database: str) -> Engine:
        ...
        print(username, quote_plus(username))
        print(password, quote_plus(password))
        print(hostname, port, database)
        return sqlalchemy.create_engine(
            f"mysql+pymysql://{quote_plus(username)}:{quote_plus(password)}@{hostname}:{port}/{database}?charset=utf8mb4", pool_pre_ping=True
        )
