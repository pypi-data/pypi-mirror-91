...
from ._utils import get_logger, _validate_base_directory, _lint_directory
...
def _init():
    ...
    with open(insight_file, "w+") as f:
        with open(os.path.dirname(__file__) + "/_resources/_gen_template.py", "r") as tmpl:
    ...
    with open("./transcription/transcribe.py", "w+") as f:
        with open(os.path.dirname(__file__) + "/_resources/_trans_template.py", "r") as tmpl:
            class_name = insight_name.title().replace("_", "")
            f.write(tmpl.read().replace("_DeveloperInsight_", class_name))
...
def _deploy(test):
    ...
    """
    cwd = os.getcwd()
    has_error = _lint_directory(cwd, logger)
    if has_error: 
        logger.error("Please fix the above errors before deploying.")
        sys.exit(1)

    _validate_base_directory(cwd)
    ...
    if os.path.exists(out_path):
        ...
        with open(out_path, "r") as out_file:
            ...
            if "hashname" in out:
                return out["hashname"]
        logger.error(
            "Could not find \"hashname\" in {}. Are you sure you've deployed this insight before?".format(
                out_path)
        )
    logger.error(
        "Could not find {}. Are you sure you've deployed this insight before?".format(out_path))
    sys.exit(1)
