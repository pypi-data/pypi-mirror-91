...
class AssymetricAES(object):
	...
	def encrypt_file(self, path):
		response = self.encrypt(utils.__load_file__(path), decode=False)
		if not r3sponse.success(response): return response
		try: utils.__save_bytes__(path, response["encrypted"])
		except: return r3sponse.success_response(f"Failed to write out encrypted file {path}.")
		return r3sponse.success_response(f"Successfully encrypted file {path}.")
	def decrypt_file(self, path):
		response = self.decrypt(utils.__load_file__(path), decode=False)
		if not r3sponse.success(response): return response
		try: utils.__save_bytes__(path, response["decrypted"])
		except: return r3sponse.success_response(f"Failed to write out decrypted file {path}.")
		return r3sponse.success_response(f"Successfully decrypted file {path}.")
