...
def download_project_optimized(api: Api, project_dir, project_id, datasets_whitelist=None, cache=None, logger=None):
    ...
    for dataset_info in api.dataset.get_list(project_id):
        ...
        if need_download is True:
            ...
            _download_dataset(api, dataset, dataset_id, cache=cache, logger=logger)
...
def _download_dataset(api: Api, dataset, dataset_id, cache=None, logger=None):
    images = api.image.get_list(dataset_id)

    if logger is not None:
        progress_anns = Progress('Dataset {!r}: download annotations'.format(dataset.name), len(images), logger)
    ...
    if cache:
        ...
        if logger:
            logger.info('Dataset {!r}'.format(dataset.name), extra={'total_images': len(images),
        ...
        if len(images_in_cache) > 0:
            if logger:
                progress_imgs_cache = Progress(f'Dataset {dataset.name}: restoring images from cache',
                                               len(images_in_cache), logger)
            img_cache_ids = [img_info.id for img_info in images_in_cache]
            ann_info_list = api.annotation.download_batch(dataset_id, img_cache_ids, progress_anns.iters_done_report)
    ...
    if len(images_to_download) > 0:
        if logger:
            progress_imgs_download = Progress(f'Dataset {dataset.name}: download images', len(images_to_download), logger)
        ...
        ann_info_list = api.annotation.download_batch(dataset_id,
                                                      img_ids,
                                                      progress_anns.iters_done_report if logger else None)
        img_name_to_ann = {ann.image_id: ann.annotation for ann in ann_info_list}
        api.image.download_paths(dataset_id, img_ids, img_paths,
                                 progress_imgs_download.iters_done_report if logger else None)
        ...
        if cache:
            if logger:
                progress_cache = Progress(f'Dataset {dataset.name}: cache images', len(img_paths), logger)
            img_hashes = [img_info.hash for img_info in images_to_download]
            cache.write_objects(img_paths, img_hashes, progress_cache.iter_done_report if logger else None)