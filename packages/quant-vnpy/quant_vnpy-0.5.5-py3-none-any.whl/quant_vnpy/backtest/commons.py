...
from quant_vnpy.db.orm import set_account_backtest, BacktestStrategyStats, BacktestStrategyStatusEnum
...
def run_backtest(
        strategy_class: Union[typing.Type[CtaTemplate], Type[StrategyTemplate]],
        multi_symbols: bool,
        engine_kwargs: dict,
    ...
    if output_available_only and not statistics['available']:
        ...
        # 将回测结果保存到数据库
        if save_stats:
            BacktestStrategyStats.update_stats(
                strategy_kwargs,
                engine_kwargs,
                statistics,
            )
...
def bulk_backtest(
        ...
        strategy_param_dic_list: Union[typing.Dict[str, list], List[typing.Dict[str, list]]],
        available_backtest_params_check_func: Optional[typing.Callable] = None,
        file_name_func: Optional[typing.Callable] = None,
    ...
    logger.info(
        ...
        f'{len(param_name_list)} 个参数，总计 {backtest_params_count:,d} 个回测')
    # 再次获得迭代器进行循环
    ...
    if backtest_params_count == 0:
        ...
        for key, df in zip(keys, df_list):
            csv_file_path = get_output_file_path(
                f'{vt_symbol_str}_{key}_{date_2_str(date.today())}.csv', root_folder_name=root_folder_name)
            df['backtest_status'] = BacktestStrategyStatusEnum.Unavailable.value
            df[df['available']].to_csv(csv_file_path, encoding='utf-8')
        ...
        try:
            ...
            result_df['backtest_status'] = BacktestStrategyStatusEnum.Unavailable.value
