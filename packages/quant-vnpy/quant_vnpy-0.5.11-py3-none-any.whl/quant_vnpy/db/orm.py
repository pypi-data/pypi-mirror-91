...
class BacktestStrategyStatusEnum(enum.IntEnum):
...
class BacktestStrategyStats(Model):
    strategy_class_name: str = CharField(max_length=55, help_text="策略的类名称")
    id_name: str = CharField(max_length=250, help_text="生成相关图片等文件时使用的文件名头部标示")
    ...
    backtest_status: int = SmallIntegerField(
        constraints=[SQL(f'DEFAULT {BacktestStrategyStatusEnum.Unavailable.value}')],
    ...
    class Meta:
        ...
        primary_key = CompositeKey('strategy_class_name', 'id_name', 'symbols', 'cross_limit_method')
    ...
    def update_backtest_status(data_dic_list: typing.List[typing.Dict]):
        try:
            ...
            BacktestStrategyStats._meta.database.close()
...
class StrategyStatus(Model):
    ...
    * status 策略状态
    ...
    backtest_status: int = SmallIntegerField(default=BacktestStrategyStatusEnum.CompareTradeData.value)
    ...
    def register_strategy(strategy_name, status: int, symbols: str, strategy_settings: dict):
        try:
            ...
            ret_data = StrategyStatus.insert(
                ...
                backtest_status=BacktestStrategyStatusEnum.CompareTradeData.value
            ).on_conflict(
                ...
                update={
                    ...
                    StrategyStatus.backtest_status: BacktestStrategyStatusEnum.CompareTradeData.value,
                }
            ).execute()
            logger.debug("%s[%s] %s status=%d", user_name, broker_id, strategy_name, status)
            return ret_data
