...
def test_config_get_cli_name(sandbox: Sandbox):
    with sandbox.push_xdg_config_home():
        init("symflow")
        assert Config.get_cli_name() == "sym-flow-cli"

        init("unknown")
        assert Config.get_cli_name() == "unknown"

        # sym init must be done last to make sure cli stays in its own
        # original state. otherwise, this will bleed into other tests.
        init("sym")
        assert Config.get_cli_name() == "sym-cli"
