...
def execute():
    ...
    for i in range(len(paths)):
        ...
        R, G, B = (
            color_converted_img_uint8[:, :, 0],
            color_converted_img_uint8[:, :, 1],
            color_converted_img_uint8[:, :, 2],
        )
        binary_output = np.full_like(R, 255)
        binary_output[(R >= threshold) & (G >= threshold) & (B >= threshold)] = 0
        ...
        print(new_path)
        final_image = Image.fromarray(binary_output, "L")
