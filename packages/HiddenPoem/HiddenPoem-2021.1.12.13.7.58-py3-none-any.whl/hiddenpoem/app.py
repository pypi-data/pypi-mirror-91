...
def get_poem_map():
    ...
    return dict(zip(df.char, df.poems.map(list)))
...
def poem_gen(sent):
    r = []
    for char in sent:
        poems = poem_map.get(char, [char + "".join(np.random.choice(new_chars, 6))])  # 造诗
        r.append(np.random.choice(poems))
    return r


# print(poem_gen("夸夸我进口零食xx"))
app = FastAPI()
