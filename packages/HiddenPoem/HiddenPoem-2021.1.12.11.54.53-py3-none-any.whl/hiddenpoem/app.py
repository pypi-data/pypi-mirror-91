...
poem7 = {k: [i for i in v if len(reg_chinese.sub('', i)) == 7] for k, v in poem7.items()}
poem5 = {k: [i+i[-1]*2 for i in v if len(reg_chinese.sub('', i)) == 5] for k, v in poem5.items()}
...
def poem_gen(sent):
    ...
    for char in sent:
        ...
        if unique_poems:
            ...
            poem = char + "".join(np.random.choice(new_chars, 6))  # 造诗
