...
class LikeModule(object):
    ...
    def computeLikelihood(self, ctx):
        ...
        logl = np.log(1 / np.sqrt(6.28 * np.linalg.det
        (self.cov))) - np.dot(diff, np.dot(self.cov_inv, diff.T)) / 2.
        logl = logl / self.div
        return logl
...
class ComplexLikeModule(object):
	...
	"""
        ...
        for i in range(len(self.non_na_index)):
            logl[self.non_na_index[i]] = -0.5 * np.dot((diff[i] * 1. / (self.cov[i])), diff[i].T) + np.log(
                1. / np.sqrt(6.28 * np.linalg.det(np.eye(len(self.k1)) * self.cov[i])))
        ...
        if self.region == 'unique':
            ...
            raise NameError('use correct region name! see docstring.')
