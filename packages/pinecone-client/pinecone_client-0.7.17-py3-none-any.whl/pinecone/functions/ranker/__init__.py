    ...
    def merge_query_msg(orig: 'core_pb2.Request', msg: 'core_pb2.Request') -> 'core_pb2.Request':
        for orig_matches, new_matches in zip(orig.query.matches, msg.query.matches):
            ...
            if orig.query.include_data:
                orig_embeddings = load_numpy(orig_matches.data)
                new_embeddings = load_numpy(new_matches.data)
                if len(new_embeddings) == 0:
                    pass
                elif len(orig_embeddings) == 0:
                    orig_matches.data.CopyFrom(dump_numpy(new_embeddings))
                else:
                    orig_matches.data.CopyFrom(dump_numpy(
                        np.concatenate((orig_embeddings, new_embeddings))))
