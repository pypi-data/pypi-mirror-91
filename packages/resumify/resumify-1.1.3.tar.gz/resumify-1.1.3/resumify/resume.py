...
class TwoColumnTemplate(FPDF):
    ...
    def __init__(self):
        ...
        # Add ubuntu fonts
        self.add_font('Ubuntu-LightItalic', '', os.path.join(BASE_DIR, 'TrashFiles/fonts/Ubuntu/Ubuntu-LightItalic.ttf'), uni=True)
        self.add_font('Ubuntu-Light', '', os.path.join(BASE_DIR, 'TrashFiles/fonts/Ubuntu/Ubuntu-Light.ttf'), uni=True)
        self.add_font('Ubuntu-Bold', '', os.path.join(BASE_DIR, 'TrashFiles/fonts/Ubuntu/Ubuntu-Bold.ttf'), uni=True)
        self.add_font('Ubuntu-Medium', '', os.path.join(BASE_DIR, 'TrashFiles/fonts/Ubuntu/Ubuntu-Medium.ttf'), uni=True)
        self.add_font('Ubuntu', '', os.path.join(BASE_DIR, 'TrashFiles/fonts/Ubuntu/Ubuntu-Regular.ttf'), uni=True)
    ...
    def add_links(self, links):
        ...
        for i in range(0, len(vals), 3):
            if len(vals[i:i+3]) > 1:
                gap = (self.ttl_wid - sum([self.get_string_width(k+" : "+v[0])+2 for k, v in vals[i:i+3]]))/(len(vals[i:i+3])-1)
            else:
                gap = (self.ttl_wid - sum([self.get_string_width(k+" : "+v[0])+2 for k, v in vals[i:i+3]]))
            ...
            for k, v in vals[i:i+2]:
                ...
                self.set_text_color(*self.heading_color)
                self.cell(self.get_string_width(v[0]), 4.5, txt=v[0], border=self.border_val, ln=0, align="R", link=v[1])
