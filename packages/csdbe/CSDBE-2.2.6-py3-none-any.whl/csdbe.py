				...
				for cmd in command.comms:
					...
					if "$typing(" in cde:
						...
						if "$case" in cde:
							...
							trig=trig.replace("#serverID",message.guild.id)
							trig=trig.replace("#authorID",message.author.id)
							...
							m=m[1].split(")")[0]
							m=m.replace("#serverID",message.guild.id)
							m=m.replace("#authorID",message.author.id)
