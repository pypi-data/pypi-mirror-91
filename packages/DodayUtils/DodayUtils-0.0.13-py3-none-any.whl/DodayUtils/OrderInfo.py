...
class DodayItem:
	...
	def __add__(self, item, **kwargs):
		...
		return DodayItem(self.category, self.title, self.options, self.price*new_amount, new_amount, self.comments)
	...
	* payment_method
	...
	def db_format_to_order_info(cls, db_data, **kwargs):
		...
		for i in range(len(sorted_db_data)):
			item_list.append(DodayItem(sorted_db_data[i]["name1"], sorted_db_data[i]["name2"], \
				sorted_db_data[i]["name3with4"], sorted_db_data[i]["price"], comments=sorted_db_data[i]["name5"], \
				order_no=sorted_db_data[i]["order_no"]))
	...
	def json_format_to_order_info(cls, json_data, **kwargs):
		...
		for iidx in range(len(json_clone["name1"])):
			item_list.append(DodayItem(json_clone["name1"][iidx], json_clone["name2"][iidx], \
				json_clone["name3with4"][iidx], json_clone["price"][iidx], comments=json_clone["name5"][iidx], \
				order_no=json_clone["order_no"][iidx]))
		...
		while orders_clone != []:
			...
			for iid in range(1, len(orders_clone)):
				try:
					i2 = orders_clone[iid]
					tmp_item = i1 + i2
					new_order_list.append(tmp_item)
					orders_clone.remove(i1)
					orders_clone.remove(i2)
					break
				except:
					# continue on trying to add 
					pass
			else:
				new_order_list.append(i1)
				orders_clone.remove(i1)
				# expect the next orders_clone[0] will be different

		self.orders = copy.deepcopy(new_order_list)

	@dutils
	def assign_order_no(self, **kwargs):
		for i in range(1, len(self)+1):
			self.orders[i]["order_no"] = i
	...
	def update_special_day(self, special_day_generator, **kwargs):
		if self.get_order_type != "local-pay":
			# All online methods
			for order in self.orders:
				if order["name2"] == "special day":
					[new_base, new_topping1, new_topping2, new_topping3] = special_day_generator()
					individual_order["name3with4"]["基底"] = new_base
					individual_order["name3with4"]["加料專區"] = [new_topping1, new_topping2, new_topping3]
	...
	def tosplitted(self, **kwargs):
		...
		for i in range(1, len(self)+1):
			ind_info = self.get_info_json()
			ind_info.update(self.orders[i].tojson())
			ind_info["order_no"] = i
	...
	def tojson(self, **kwargs):
		...
		json_raw_data = {
			"order_no": [i for i in range(1, len(self)+1)],
	...
	def toprinterargs(self, **kwargs):
		...
		if stayortogo == "stay":
			...
			stayortogostr = " "
		print_order_args.append(c_char_p(stayortogostr.encode('big5')))
	...
	def get_info_json(self, **kwargs):
		return {
			...
			"comment4": self.delivery_info,
			"comment5": self["return_changes"],
			"comment7": {"one_time_code": self["one_time_code"], "deducted_point": self["deducted_point"]}
		}
