...
class DodayWebsocketServer:
	...
	def receive_msg_handler(self, client, server, message, **kwargs):
		logging.info("[WebsocketServer] Message Handle: " + message)
		response = self.msg_handler(message, self.api_context, **self.passargs)
		logging.info("[WebsocketServer] Message Handle Response: " + str(response))
		...
		if response["indicator"]:
			server.send_message_to_all(json.dumps(response["message"], ensure_ascii=False))
		else:
			server.send_message(client, json.dumps(response["message"], ensure_ascii=False))
	...
	def receiving(self, **kwargs):
		...
		while True:
			try:
				...
				self.msg_handler(message, **self.msg_parameters)
