...
class DodayWebsocketServer:
	...
	def receive_msg_handler(self, client, server, message, **kwargs):
		...
		logging.info("[WebsocketServer] Message Handle Response: " + json.dumps(response["message"], ensure_ascii=False))
		...
		if response["indicator"]:
			server.send_message_to_all(urllib.parse.quote(json.dumps(response["message"], ensure_ascii=False)))
		else:
			server.send_message(client, urllib.parse.quote(json.dumps(response["message"], ensure_ascii=False)))
