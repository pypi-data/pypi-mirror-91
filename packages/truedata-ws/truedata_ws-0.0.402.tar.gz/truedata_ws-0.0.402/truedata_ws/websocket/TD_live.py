    ...
    def handle_trade_data(self, trade_tick):
        try:
            ...
            for ticker_id in self.parent_app.symbol_mkt_id_map[symbol]:
                ...
                self.parent_app.touchline_data[ticker_id].open = float(trade_tick[6])
                self.parent_app.touchline_data[ticker_id].high = float(trade_tick[7])
                self.parent_app.touchline_data[ticker_id].low = float(trade_tick[8])
                self.parent_app.touchline_data[ticker_id].prev_close = float(trade_tick[9])
                self.parent_app.touchline_data[ticker_id].oi = int(trade_tick[10])
                self.parent_app.touchline_data[ticker_id].prev_oi = int(trade_tick[11])
                self.parent_app.touchline_data[ticker_id].turnover = float(trade_tick[12])
