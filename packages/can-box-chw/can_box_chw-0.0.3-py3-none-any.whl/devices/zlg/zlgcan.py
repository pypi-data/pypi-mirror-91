...
author: chenhewen
from ZLGCAN接口函数使用手册
desc: 我们将发送类型定义在这里，这个本应该ZLG定义在这里的，但是却没有
0=正常发送，1=单次发送，2=自发自收，3=单次自发自收
"""
ZCAN_SEND_TYPE_NORMAL = 0
ZCAN_SEND_TYPE_NORMAL_ONCE = 1
ZCAN_SEND_TYPE_SEND_TO_MYSELF = 2
ZCAN_SEND_TYPE_SEND_TO_MYSELF_ONCE = 3
...
class ZCAN(object):
    def __init__(self, zlgcan_dll):
        if platform.system() == "Windows":
            self.__dll = windll.LoadLibrary(zlgcan_dll)
        else:
...
if __name__ == "__main__":
    ...
    chn_handle_rec = can_start(zcanlib, handle, 1)
    print("rec channel handle:%d." %(chn_handle_rec))
