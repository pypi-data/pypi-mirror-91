...
class ModbusConnection:     #Singleton
    ...
    def __init__(self):
        ...
        self.__modbusClient__ = ModbusClient(method = "rtu", port="/dev/ttyUSB0", stopbits = 1, bytesize = 8, parity = 'N', baudrate= 19200, timeout=0.2, retry_on_empty=True, retry_on_invalid=True)
    ...
    def readInputRegisters(self, node, startAddress = 1000, registersToRead = 1):
        with self.__mutex__:
            logging.debug("[ModbusConnection] Node " + str(node) + ": Reading " + str(registersToRead) +" input registers starting with register "+ str(startAddress))
            try:
                self.__connectionCounter__[node] += 1
            except:
                self.__connectionCounter__[node] = 1
            ...
            if self.__modbusClient__ != None:
                if(self.__connectSocket__()):
                    ...
                    logging.warning("[ModbusConnection] Cannot read input registers from node " + str(node) + ".")
        return registers
