...
The main entrypoint is :func:`get_transport`.
"""
import importlib
import logging

import smart_pathlib.local_file

logger = logging.getLogger(__name__)

NO_SCHEME = ''

_REGISTRY = {NO_SCHEME: smart_pathlib.local_file}
_ERRORS = {}
_MISSING_DEPS_ERROR = """You are trying to use the %(module)s functionality of
smart_pathlib but you do not have the correct %(module)s dependencies installed.
Try:
    pip install smart_pathlib[%(module)s]
"""

MANDATORY_FUNCTIONS = ('exists', )


def register_transport(submodule):
    """Register a submodule as a transport mechanism for ``smart_pathlib``.
    This module **must** have:
        - `SCHEME` attribute (or `SCHEMES`, if the submodule supports multiple
        schemes)
        - `stat` function
        - `exists` function
    ...
    if hasattr(submodule, 'SCHEME'):
        ...
        raise ValueError('%r does not have a .SCHEME or .SCHEMES attribute'
                         % submodule)

    for f in MANDATORY_FUNCTIONS:
...
def get_transport(scheme):
    ...
    This submodule must have been previously registered via
    :func:`register_transport`.
    """
    global _ERRORS, _MISSING_DEPS_ERROR, _REGISTRY, SUPPORTED_SCHEMES
    expected = SUPPORTED_SCHEMES
    readme_url =\
        'https://github.com/gfournier/smart_pathlib/blob/master/README.rst'
    message = (
...
register_transport(smart_pathlib.local_file)
register_transport('smart_pathlib.gcs')

SUPPORTED_SCHEMES = tuple(sorted(_REGISTRY.keys()))
"""The transport schemes that the local installation of ``smart_pathlib``
supports."""
