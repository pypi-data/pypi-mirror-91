...
class PackInstallation:
    ...
    def on_change(self):
        ...
        # pinned worker dependencies are prioritized
    ...
    def __migrate_dirs(self, log=log):
        """
        Introduced with worker 0.10.1
        """
        if os.path.exists(self.venv_base_path) and not os.path.exists(self.venv_path):
            venv_temp_path = self.venv_base_path + "_tmp"
            log.info(
                "%s: Migrating venv from %s to %s",
                self.display_name,
                self.venv_base_path,
                self.venv_path,
            )
            os.rename(self.venv_base_path, venv_temp_path)
            os.makedirs(self.venv_base_path)
            shutil.move(venv_temp_path, self.venv_path)
