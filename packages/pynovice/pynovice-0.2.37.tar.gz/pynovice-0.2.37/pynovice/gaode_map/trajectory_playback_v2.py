...
pattern_markerlist=re.compile(r"(?<=var tripArr = ).*?(?=;$)",re.MULTILINE | re.DOTALL)
...
class Trajectory():
    def __init__(self, lnglat_list=[], marker_list=[], auto_marker=True):
        self.js=get_main_js()
        self.gps_str = json.dumps(lnglat_list)
...
def get_main_js():
    with open('pynovice_map.js','r') as f:
        main_js = f.read()
#     main_js = '''
# <!doctype html>
#     <html>
...
if __name__ == '__main__':
    ...
    traj = Trajectory(lnglat_list=gps_list,marker_list=marker_list,auto_marker=False)
