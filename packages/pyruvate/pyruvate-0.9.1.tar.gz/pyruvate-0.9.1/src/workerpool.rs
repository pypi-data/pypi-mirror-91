...
pub type WorkerPayload<T> = (WSGIRequest, Option<HTTPPersistentConnection<T>>);
...
pub struct WorkerPool<T: Connection> {
    ...
    pub reusable_connections: Receiver<(Token, HTTPPersistentConnection<T>)>,
}
...
impl<T: 'static + Connection + Sync> WorkerPool<T> {
    ...
    where
        F: 'static
            + Fn(
                ...
                SharedWSGIOptions,
                PyObject,
                Receiver<(Token, (WSGIRequest, Option<HTTPPersistentConnection<T>>))>,
                Sender<(Token, HTTPPersistentConnection<T>)>,
            )
            + Send
            + Copy,
    {
        let (input, rcv) = unbounded::<(Token, WorkerPayload<T>)>();
        let (snd, reusable_connections) = unbounded::<(Token, HTTPPersistentConnection<T>)>();
        let wp = WorkerPool {
            application,
    ...
    pub fn execute(
        ...
        token: Token,
        req: WSGIRequest,
        out: Option<HTTPPersistentConnection<T>>,
    ) -> SendResult<T> {
