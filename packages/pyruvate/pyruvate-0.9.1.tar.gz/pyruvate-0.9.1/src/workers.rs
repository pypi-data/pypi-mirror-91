...
use crate::transport::{
    broken_pipe, would_block, BlockingWrite, Connection, HTTPPersistentConnection, Listener,
    NonBlockingWrite, SetBlocking,
};
use crate::workerpool::{WorkerPayload, MARKER};

/// Returns `true` if the connection is done.
fn handle_write_event<T: Connection>(
                    ...
                    match event.token() {
                        token if event.is_writable() => {
                            ...
                            if let Some(mut resp) = responses.remove(&token) {
                                ...
                                match handle_write_event(&mut resp, py, thread_state) {
                                    Ok(done) => {
                                        if done {
                                            // s. https://docs.rs/mio/0.7.0/mio/event/trait.Source.html#dropping-eventsources
                                            if let Err(e) =
                                                poll.registry().deregister(&mut resp.connection)
                                            {
...
app = simple_app"#,
    ...
    fn test_handle_write_event() {
        ...
        match handle_write_event(&mut r, py, py_thread_state) {
            Err(e) => {
                debug!("Error: {:?}", e);
                assert!(false);
            }
