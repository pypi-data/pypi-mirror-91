...
from .my_utils import get_env_var_list


saved_formatwarning = warnings.formatwarning

test_disabled_modules = [module.strip() for module in (get_env_var_list(f"{Constants.MAGIC_CLASS_NAME_UPPER}_TEST_DISABLE_MODULES") or [])]
...
class Dependencies(object):
    ...
    dependencies: list = [
        ...
        ('requests', 'requests', OPTIONAL_TAG, "won't be able to query sources, except local cache", VERSION_IN_MODULE),
    ...
    def get_module(cls, module_name:str, package_name:str=None, version_location:str=None, dont_throw:bool=False, message:str=None):
        try:
            ...
            if module_name in test_disabled_modules:
                raise Exception("test_module_disabled")
            return module
        except Exception as error:
            cls.installed_modules[module_name] = False
            if f"{error}" == "test_module_disabled":
                test_module_disabled_message = f"module {module_name} was disabled, because specified in enironment variable {Constants.MAGIC_CLASS_NAME_UPPER}_TEST_DISABLE_MODULES"
                print(f"!!! Note: {test_module_disabled_message} !!!")
                message = f", {message}" if message else ""
                message = f"{test_module_disabled_message}{message}"
            if dont_throw:
                return None
    ...
    def warn_missing_dependencies(cls, options:Dict[str,Any]={})->None:
        ...
        extras_names = [name.strip() for name in (get_env_var_list(f"{Constants.MAGIC_CLASS_NAME_UPPER}_EXTRAS_REQUIRE") or [])]
        if extras_names:
            cls.warn_unknown_extras_require_name(extras_names)
            cls.extras_require_packages = list_union(*[EXTRAS_REQUIRE.get(name, []) for name in extras_names])
        else:
        ...
        install_package_names = list_union(cls.install_requires_package_names, cls.extras_require_package_names)

        for item in cls.dependencies:
            cls._import_warn(install_package_names, *item)
