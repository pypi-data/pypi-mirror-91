...
class ClusterBuilder:
    ...
    def _get_best_head_avg_count(self, cluster_items):
        ...
        # TODO add embedding as tuple in head to avoid having to regenerate it 
        # in `create_clusters_from_queries`
        # return cluster_items[max_avg_similarity_i][0], cluster_items[max_avg_similarity_i][2]), max_avg_similarity, n
    ...
    def _get_answers_from_queries(self, query_list):
        ...
        # { question: [(answer, embedding, sent-id)] }
        return query_dic
    ...
    def _get_mini_clusters(self, embeddings, sents, orig_ids, min_similarity):
        ...
        Parameters:
            ...
            sents: list of answers for the current question
            orig_ids: list of the sentence ids for each answer
            min_similarity: 
            
        Returns:
            good_clusters: { head_answer_sent: [(answer, sent_id, embedding)] }
        """
    ...
    def _get_best_clusters(self, labels, embeddings, answers, ids, min_similarity_start):
        ...
        Parameters:
            ...
            ids: list of sentence ids for each answer

        Returns:
            all_mini_clusters: { head_answer_sent: [(answer, sent_id, embedding)] }   
            ...
            if len(query_dic[q]) > 0:
                ...
                while True:
                    ...
                    if len(all_mini_clusters) > 0:
                        ...
                        # TODO embedding could be taken directly from the head tuple if 
                        # that is added to the return from `_get_best_head_avg_count`
                        # all_mini_clusters_keys, all_mini_cluster_embeddings = map(list, zip(*all_mini_clusters.keys()))
