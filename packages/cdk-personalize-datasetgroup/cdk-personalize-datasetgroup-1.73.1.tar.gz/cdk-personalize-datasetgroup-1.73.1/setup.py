import json
import setuptools

kwargs = json.loads(
    """
{
    "name": "cdk-personalize-datasetgroup",
    "version": "1.73.1",
    "description": "Create an AWS Personalize Dataset Group, Interaction Schema, and more using the AWS CDK",
    "license": "Apache-2.0",
    "url": "https://github.com/kcwinner/cdk-personalize-datasetgroup.git",
    "long_description_content_type": "text/markdown",
    "author": "Ken Winner<kcswinner@gmail.com>",
    "bdist_wheel": {
        "universal": true
    },
    "project_urls": {
        "Source": "https://github.com/kcwinner/cdk-personalize-datasetgroup.git"
    },
    "package_dir": {
        "": "src"
    },
    "packages": [
        "cdk_personalize_datasetgroup",
        "cdk_personalize_datasetgroup._jsii"
    ],
    "package_data": {
        "cdk_personalize_datasetgroup._jsii": [
            "cdk-personalize-datasetgroup@1.73.1.jsii.tgz"
        ],
        "cdk_personalize_datasetgroup": [
            "py.typed"
        ]
    },
    "python_requires": ">=3.6",
    "install_requires": [
        "aws-cdk.aws-iam>=1.73.0, <2.0.0",
        "aws-cdk.aws-lambda>=1.73.0, <2.0.0",
        "aws-cdk.core>=1.73.0, <2.0.0",
        "aws-cdk.custom-resources>=1.73.0, <2.0.0",
        "constructs>=3.2.27, <4.0.0",
        "jsii>=1.16.0, <2.0.0",
        "publication>=0.0.3"
    ],
    "classifiers": [
        "Intended Audience :: Developers",
        "Operating System :: OS Independent",
        "Programming Language :: JavaScript",
        "Programming Language :: Python :: 3 :: Only",
        "Programming Language :: Python :: 3.6",
        "Programming Language :: Python :: 3.7",
        "Programming Language :: Python :: 3.8",
        "Programming Language :: Python :: 3.9",
        "Typing :: Typed",
        "Development Status :: 5 - Production/Stable",
        "License :: OSI Approved"
    ]
}
"""
)

with open("README.md", encoding="utf8") as fp:
    kwargs["long_description"] = fp.read()


setuptools.setup(**kwargs)
