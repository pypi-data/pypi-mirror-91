...
class Address:
    ...
    def serialize(self, address_offset=0):
        # fixed address type
        ...
        if self.access is not None:
            ...
            access_len = up_align(len(access) + 1, 2)  # align to 16bit
            result += b"AC"
            result += int2bytearray(access_len, 2)
            result += access
            result += int2bytearray(0x00, access_len - len(access))  # zero termination + padding with zero

        # if self.width is not None:
        #     result += b"WI"
        #     result += int2bytearray(self.width, 2)
