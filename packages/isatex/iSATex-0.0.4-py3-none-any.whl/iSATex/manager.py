...
from const import (ABOUT_MENU_ITEM, COLOR_THEME, COLOR_THEME_FOLDER_PATH,
                   COLOR_THEME_LIST, COLORMAP, CUSTOM, DATA_BUFFER_SIZE,
                   DATA_MANAGER, DECODE, DECODE_ENCODING,
...
class IOManager(Singleton):
    ...
    def SaveColorThemeList(self, color_theme_list: Iterable[Dict]):
        ...
        """
        folder_path = abspath(join(__file__, '../', PLUGIN_FOLDER_PATH))
    ...
    def SaveProject(self, project: Project):
        ...
        contents = dumps(project, separators=(',', ':'), cls=IOManager.iSATexJsonEncoder)
    ...
    def OnEvent(self, event):
        ...
        if event_type == wxEVT_PROJECT_NEW:
            ...
            for spectrum_func_instance in SpectrumFunctionContainerBase._instance_list:
                ...
                max_value = max(max_value, sys.getrefcount(spectrum_func_instance))
                min_value = min(min_value, sys.getrefcount(spectrum_func_instance))
...
class ColorManager(Singleton):
    ...
    def GetMainSelectionColor(self) -> str:
        ...
        :rtype: str
        """
        return self.GetSelectedColorTheme()[MAIN_SELECTION_COLOR]
