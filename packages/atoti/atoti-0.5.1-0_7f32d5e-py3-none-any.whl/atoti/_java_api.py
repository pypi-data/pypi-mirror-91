...
from .config._aws import AwsConfiguration, AwsKeyPair, AwsKmsConfiguration
from .config._azure import AzureConfiguration
...
class JavaApi(metaclass=ApiMetaClass):
    ...
    def _create_cloud_encryption(
        self,
        key_id: Optional[str],
        private_key: Optional[str],
        public_key: Optional[str],
    ):
        """Create CloudEncryption java object."""
        package: Any = self.gateway.jvm.io.atoti.loading.cloud  # type: ignore
        return package.CloudEncryption(
            key_id,
            private_key,
            public_key,
        )

    def set_aws_config(self, aws_config: AwsConfiguration):
        """Set the AWS config."""
        client_encryption = None
        if isinstance(aws_config.client_side_encryption, AwsKmsConfiguration):
            client_encryption = self._create_cloud_encryption(
                aws_config.client_side_encryption.key_id, None, None
            )
        if isinstance(aws_config.client_side_encryption, AwsKeyPair):
            client_encryption = self._create_cloud_encryption(
                None,
                aws_config.client_side_encryption.private_key,
                aws_config.client_side_encryption.public_key,
            )
        self.java_api.setAwsConfig(aws_config.region, client_encryption)

    def set_azure_config(self, azure_config: AzureConfiguration):
        """Set the Azure config."""
        client_encryption = self._create_cloud_encryption(
            azure_config.client_side_encryption.key_id,
            azure_config.client_side_encryption.private_key,
            azure_config.client_side_encryption.public_key,
        )
        self.java_api.setAzureConfig(client_encryption)
