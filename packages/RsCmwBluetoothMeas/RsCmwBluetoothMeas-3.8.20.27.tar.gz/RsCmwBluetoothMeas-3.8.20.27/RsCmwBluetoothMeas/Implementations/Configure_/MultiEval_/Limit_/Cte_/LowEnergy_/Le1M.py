...
class Le1M:
	...
	def get_freq_offset(self) -> FreqOffsetStruct:
		...
		Snippet: value: FreqOffsetStruct = driver.configure.multiEval.limit.cte.lowEnergy.le1M.get_freq_offset() \n
		...
		"""
		return self._core.io.query_struct('CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:CTE:LENergy:LE1M:FOFFset?', self.__class__.FreqOffsetStruct())

	def set_freq_offset(self, value: FreqOffsetStruct) -> None:
		"""SCPI: CONFigure:BLUetooth:MEASurement<Instance>:MEValuation:LIMit:CTE:LENergy:LE1M:FOFFset \n
		Snippet: driver.configure.multiEval.limit.cte.lowEnergy.le1M.set_freq_offset(value = FreqOffsetStruct()) \n
		Sets/gets the frequency offset limit for the CTE portion. Commands for uncoded LE 1M PHY (..:LE1M..) and LE 2M PHY (..
		:LE2M..) are available. \n
