...
from geodesic.features import FeatureCollection
...
class Dataset:
    ...
    def _repr_html_(self,add_style=True):
        ...
        for p in self._spec['stac'].get('providers', []):
            pl = provider_template.format(name=p['name'], url=p['url'], roles=', '.join(p['roles']))
            providers += pl
    ...
    def query(self, bbox=None, datetime=None, **kwargs):
        ...
        if datetime is not None:
            params['datetime'] = '/'.join([d.isoformat() for d in datetime])
        ...
        collection = FeatureCollection(
            obj=res, 
            _ds_type=self._spec['type'], 
            _ds_subtype=self._spec['subtype']
        )
...
class DatasetList:
    def __init__(self, datasets):
        self.ddict = {
            dataset.name: dataset for dataset in datasets
        }
