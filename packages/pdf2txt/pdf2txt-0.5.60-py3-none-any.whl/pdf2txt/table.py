...
class Table(object):
    ...
    def get_edges(self):
        ...
        hh = utils.filter_edges(self.page.edges, "h")
        ...
        if len(utils.get_partially_overlapping_objects(utils.get_BoundingBox_from_objects([t for line in self.textlines for t in line]), h)) >= len(self.textlines)-2:
    ...
    def horizontally_within_lines(self, line_bottom, line_top):
        ...
        if line_top is not None:
            ...
            ll = [line for line in self.textlines if line[0].bottom > line_bottom.bottom]
        return ll

    def extract(self):

        if 2 < len(self._edges_h) <= 1.6 * (len(self.textlines)+1):
    ...
    def _fix_header_rows(self, header, tolerance=5):
        ...
        for i in reversed(range(1, len(header))):
            ...
            for token in header[i - 1]:
                if token.left - tolerance > min(t.left for t in header[i]) or len(header[i - 1]) == len(
                        header2[-1]):  # not left justified or as many header as the last row
                    for t in header[i - 1]:
...
class TableFinder(object):
    ...
    def guess_tables(self):
        horizontal_lines = [line for line in self.page.horizontal_lines if
