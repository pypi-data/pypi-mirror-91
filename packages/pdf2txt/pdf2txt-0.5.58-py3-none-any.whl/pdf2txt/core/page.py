...
#                    print(table.to_string())
    ...
    def fix_lines_for_table(self, paragraph):
        paragraph=[sorted(p, key=lambda x: x.left) for p in paragraph]
        paragraph=sorted(paragraph, key=lambda x: x[0].bottom)

        z=zip(paragraph[1:], paragraph[:-1])
        separation=[a[0][0].bottom-a[1][0].bottom for a in z]
        average_separation=mean(separation)

        columns=cluster_objects([t for p in paragraph for t in p], "left", tolerance=2)
        for i, p in enumerate(paragraph):
            if i ==0:
                continue
            if len (p)==1 and separation[i-1]<=average_separation/2:
                print(p)
