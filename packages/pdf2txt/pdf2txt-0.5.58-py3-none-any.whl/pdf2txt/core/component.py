...
class Component(object):
    ...
    @property
    def rects(self):
        return [utils.BoundingBox(**rect) for rect in self.objects.get("rect", [])]#+[c for c in self.curves if c.width<100 and c.height<100]
    ...
    def lines(self):
        ...
        self.__lines= [utils.BoundingBox(**line) for line in self.objects.get("line", [])]+[rect for rect in self.rects+self.curves if rect.height<1 and rect.width>10]+[rect for rect in self.rects+self.curves if rect.width<1 and rect.height>10]

        h_lines=utils.cluster_objects([l for l in self.__lines if l.width>l.height], "top", 0.5)
        v_lines=utils.cluster_objects([l for l in self.__lines if l.width<l.height], "left", 0.5)
