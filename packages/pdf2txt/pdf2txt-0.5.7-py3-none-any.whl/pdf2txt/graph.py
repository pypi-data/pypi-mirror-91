...
def rects_to_edges(all_rectangles, words, rect_threshold=4):
    ...
    rectangles = sorted(
        [rect for rect in all_rectangles if 200 > rect.width > 0.5 and 200 > rect.height > 0.5], key=lambda l: l.top)

    # Find rectangles that share the same left, right, or centerpoints
    by_left = cluster_objects_multi(rectangles, "left", "height", 0.2)
    # in case of vertcal charts
    by_bottom = cluster_objects_multi(rectangles, "bottom", "width", 0.2)
    by_top = cluster_objects_multi(rectangles, "top", "width", 0.2)
...
def rects_to_edges_bk(rectangles, words, rect_threshold=4):
    """
