...
def convertToOBJ(dataFull,t,background=0,factor=4,Smooth=True,Decimate=True,Reduction=True,center=[0,0,0],VoxelSize=[1,1,1],maxNumberOfThreads=40,cells_updated=None,path_write=None): ####  CONVERT SEGMENTATION IN MESH
        global _dataToConvert
            ...
            if tc.polydata.GetPoints() is not None:
                ...
                if not tc.polydata.GetPoints() is None :
                    for p in range(tc.polydata.GetPoints().GetNumberOfPoints()):
                        ...
                        obj+='v ' + str(v[0]*factor*VoxelSize[0]-center[0]) +' '+str(v[1]*factor*VoxelSize[1]-center[1]) +' '+str(v[2]*factor*VoxelSize[2]-center[2])+'\n'
