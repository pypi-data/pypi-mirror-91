...
class ParametersCOSIM:
    ...
    def makeAllFolders(self, N_LEDET = 1):
        ...
        if N_LEDET == 1:
            ...
            if not os.path.isdir(os.path.join(nameFolderLedetModel, "LEDET", self.nameMagnet, "Input")):
                os.mkdir(os.path.join(nameFolderLedetModel, "LEDET", self.nameMagnet, "Input"))
            if not os.path.isdir(os.path.join(nameFolderLedetModel,"LEDET", self.nameMagnet, "Input","Initialize variables")):
                os.mkdir(os.path.join(nameFolderLedetModel,"LEDET",self.nameMagnet,"Input","Initialize variables"))
                f = open(os.path.join(nameFolderLedetModel,"LEDET",self.nameMagnet,"Input","Initialize variables", " .gitkeep"), "w")
                f.close()
            if not os.path.isdir(os.path.join(nameFolderLedetModel, "LEDET", self.nameMagnet, "Input","Control current input")):
                os.mkdir(os.path.join(nameFolderLedetModel,"LEDET",self.nameMagnet,"Input","Control current input"))
                f = open(os.path.join(nameFolderLedetModel,"LEDET",self.nameMagnet,"Input","Control current input", " .gitkeep"), "w")
                f.close()
            if not os.path.isdir(os.path.join(nameFolderLedetModel , "Field maps")):
                os.mkdir(os.path.join(nameFolderLedetModel,"Field maps"))
            if not os.path.isdir(os.path.join(nameFolderLedetModel, "Field maps", self.nameMagnet)):
                os.mkdir(os.path.join(nameFolderLedetModel, "Field maps", self.nameMagnet))
            if not os.path.isdir(os.path.join(nameFolderLedetModel, "LEDET", self.nameMagnet ,"Output")):
                os.mkdir(os.path.join(nameFolderLedetModel, "LEDET", self.nameMagnet, "Output"))
    ...
    def prepareLEDETFiles(self, files, N_PAR=1):
        ...
        if type(files) == list:
            for i in range(len(files)):
                ...
                if self.executionOrder[i+1] == self.executionOrder[i]:
    ...
    def prepareSingleLEDETFile(self, file):
        ...
        for i in range(0,len(tV),3):
            steps = steps+ (abs(tV[i])-abs(tV[i+2]))/tV[i+1]
