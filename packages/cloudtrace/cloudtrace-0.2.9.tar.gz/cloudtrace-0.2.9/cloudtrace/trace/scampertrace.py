...
def cmd_ping(proto, pps, tmp, write, ftype):
    if proto == 'icmp':
        proto = 'icmp-echo'
    elif proto == 'udp':
        proto = 'udp'
        ...
        raise Exception('Unknown proto {}'.format(proto))
    cmd = 'sudo scamper -O {type} -p {pps} -c "ping -P {proto}" -f {infile} {write}'.format(type=ftype, pps=pps, proto=proto, infile=tmp, write=write)
    return cmd
...
def main():
    ...
    parser.add_argument('-e', '--extension', choices=['warts', 'jsonl'])
    ...
    while args.cycles == 0 or cycle < args.cycles:
        ...
        try:
            ...
            if args.method == 'trace':
                ...
                cmd = cmd_ping(args.proto, args.pps, tmp, write, ftype)
            else:
