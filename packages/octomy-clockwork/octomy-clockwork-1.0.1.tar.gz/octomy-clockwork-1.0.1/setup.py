...
def get_local_git_branch():
    try:
        ...
        print(f"No git repo found, returning no branch")
        return None
    debug_repo(repo)
    ...
    for branch in repo.branches:
        if branch.commit == repo.head.commit:
            ...
            break;
    if closest_branch:
        print(f"Using branch = {closest_branch} from repo branches")
    else:
        # There might be no branch, for example in gitlab, so we look at remote ref
        remote = repo.remote()
        for ref in remote.refs:
            if ref.commit == repo.head.commit:
                ...
                break;
        if closest_branch:
            print(f"Using branch = {closest_branch} from remote refs")
        else:
            # Fall back to commit hash if all else fails
            closest_branch = repo.head.commit
            # Return None when we have no good alternative
            if closest_branch:
                print(f"Could not find branch, falling back to commit hash {closest_branch}")
            else:
                print("Best effort did not produce a valid branch")
                return None
    return str(closest_branch)
...
def get_git_branch():
    # Prefer parsing branch from current git repo
    branch = get_local_git_branch()
    # Next try getting branch from environment variable
    if not branch:
        branch = get_git_branch_from_env()
    # Fall back to a "dummy" branch
    if not branch:
        branch="unknown"
    return branch
...
def generate_version_string(version = None, branch = None):
    ...
    branch = get_git_branch() if branch is None else branch
    full_version = ""
