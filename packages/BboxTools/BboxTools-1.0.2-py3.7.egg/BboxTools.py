...
class Bbox2D(object):
    ...
    def include(self, other):
        ...
        if type(other) == np.ndarray:
            if len(other.shape) == 1 and other.size == 2:
                return self.include((other[0], other[1]))
            if len(other.shape) == 2 and other.shape[1] == 2:
                return np.logical_and(
                    np.logical_and(other[:, 0] > self.bbox[0][0], other[:, 0] < self.bbox[0][1]),
                    np.logical_and(other[:, 1] > self.bbox[1][0], other[:, 1] < self.bbox[1][1])
                )
