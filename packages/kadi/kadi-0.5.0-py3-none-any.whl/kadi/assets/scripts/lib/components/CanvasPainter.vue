...
<template>
  <div>
    <div ref="container">
      ...
      <div class="card canvas-container" ref="canvasContainer">
        <canvas @mousedown="mousedown" @mousemove="mousemove" @mouseup="mouseup" @mouseout="mouseup" ref="canvas">
        </canvas>
      </div>
...
export default {
  ...
  methods: {
    ...
    updateCoordinates(event) {
      ...
      this.currX = event.clientX - this.canvas.getBoundingClientRect().left;
      this.currY = event.clientY - this.canvas.getBoundingClientRect().top;
    },
    ...
    resizeVisibleCanvas() {
      // Provide a fallback width, in case the container is somehow not rendered yet.
      this.canvas.width = this.$refs.canvasContainer.getBoundingClientRect().width || 512;
      if (this.isFullscreen()) {
    ...
    mouseup() {
      if (this.mouseDown && (this.tool === 'pen' || this.tool === 'eraser')) {
