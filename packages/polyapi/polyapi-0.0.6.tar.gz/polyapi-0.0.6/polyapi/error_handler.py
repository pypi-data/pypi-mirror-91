...
def authorization_checks(request: Dict, r: requests.models.Response) -> Tuple:
    """
    Проверки авторизации
    :param request: (Dict) запрос
    :param r: <class 'requests.models.Response'>
    :return: (Tuple) session_id, uuid (если нет ошибок)
    """
    response = r.json()

    resp_queries = response.get("queries")
    resp_queries = next(iter(resp_queries))
    resp_command = resp_queries.get("command")
    resp_code = resp_command.get("plm_type_code")

    request_queries = request.get("queries")
...
def checks(self, func_name, *args):
    ...
    if func_name == "move_dimension":
        ...
        if (scenario_id is None) and (scenario_name is None):
            raise ValueError("Нужно ввести либо uuid сценария, либо название сценария!")
        return True
