...
class Executor:
    ...
    def execute_request(self, params: Union[Dict, str], method: str = "POST") -> [
        ...
        if method == "GET":
            ...
            logging.info("Response: %s", r.status_code)
            return r
        elif method == "PUT":
            cookies = {'session': self.session_id}
            ...
            try:
                response = r.json()
                logging.info("Headers: %s", r.request.headers)
                if request_code == 203 and request_state == 6:
                ...
                logging.exception(e)
                logging.info("Deleting Invalid control characters from response...")
                str_response = r.text
