...
class JobValidator(object):
    ...
    def validate(self, jobs: JobsCollection):
        ...
        validation_results = [
            self.validate_handlers("pre", jobs),
            self.validate_handlers("input", jobs),
            self.validate_handlers("processor", jobs),
            self.validate_handlers("output", jobs),
        ]
    ...
    def validate_handlers(self, attr: str, jobs: list):
        ...
        for job in jobs:
            job_config: JobConfigSection
            for job_config in getattr(job, job_attribute):
                handler: AbstractHandler = handlers.get_by_code(job_config.code)
                result = handler.validate_job_config(job_config)
                if result != "":
                    return result
