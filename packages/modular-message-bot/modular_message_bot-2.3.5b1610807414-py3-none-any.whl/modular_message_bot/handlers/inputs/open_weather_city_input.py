...
class OpenWeatherCityInput(AbstractSimpleInputHandler):
    ...
    def validate_job_run_var(self, job_run_var: JobRunVar):
        ...
        if (
            string_value.count(self.WEATHER_BY_TEXT_SIMPLE) <= 0
    ...
    def run_input(self, parameters: dict, job_run_vars_collection: JobRunVarCollection):
        ...
        if "query" in parameters:
            ...
            if response.status_code != 200:
                raise Exception(f"Open weather failed '{response.status_code}'\n'{response.content}'")
        ...
        for var, jq_query in jq_vars.items():
            jq_result = jq.compile(jq_query).input(weather_results)
            value = str(jq_result.text()).strip('"')
