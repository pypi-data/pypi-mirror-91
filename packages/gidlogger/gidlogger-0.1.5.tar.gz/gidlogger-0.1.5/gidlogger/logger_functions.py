...
def _each_run_file_rotate_handler(in_file_name, in_back_up):
    ...
    formatter = CustomFormatter(fmt='{asctime}|{levelname:^9}|{threadName:^25}|{lineno:^7}| {name:<30}|{funcName:^25}||--> {message}', datefmt=_date_format, style='{')
...
def std_namer(name):
    _nameparts = name.replace('.log', '').split('.')
    _path, _basename = pathmaker(_nameparts[0]).rsplit('/', 1)
    extra_nameparts = '_'.join([_nameparts[i] for i in range(len(_nameparts)) if i != 0])
    return pathmaker(_path, 'old_logs', f'{_basename}_{extra_nameparts}.log')
