...
class ComprehensibilityAnalysis:
    ...
    def get_features(self, function, dataset, num_datapoints):
        ...
        if len(features.shape) != 2:
            raise ValueError("Features not in correct format: (n_samples, n_features)")
        return features
    ...
    def permutation_importance(self, feature_names, plot=False, title='Average Feature Importances',
                               axis_title='Features'):

        def score(X, y):
            X = self.interface.from_numpy(X)
            y_pred = self.interface.predict(X)
            y_pred = self.interface.to_numpy(y_pred)
            metric = self.metric(y, y_pred)
            return metric

        x_test = self.interface.to_numpy(self.x_test)
        y_test = self.interface.to_numpy(self.y_test)
        base_score, score_decreases = get_score_importances(score, X=x_test, y=y_test)
    ...
    def partial_plot(self, feature_names):
        """
        Scipy only
        :param feature_names:
        :return:
        """
        x_test_numpy = self.interface.to_numpy(self.x_test)
        print(x_test_numpy.shape)
        print(feature_names)
        df = pd.DataFrame(x_test_numpy, columns=feature_names)
        print(df)
        pdp_goals = pdp.pdp_isolate(model=self.model, dataset=df, model_features=feature_names,
                                    feature='0')

        pdp.pdp_plot(pdp_goals, '0')
        plt.show()
    ...
    def integrated_gradients(self, index):
        img_flatten = self.x_test[index].unsqueeze(0)
        img_flatten.requires_grad = True
        ig = IntegratedGradients(self.model)
        self.model.zero_grad()
        attr_ig, delta = ig.attribute(img_flatten, target=self.y_test[index].item(), return_convergence_delta=True)
        attr_ig = np.transpose(attr_ig.squeeze().cpu().detach().numpy(), (1, 2, 0))
        print('Approximation delta: ', abs(delta))
        original_image = np.transpose((self.x_test[index].cpu().detach().numpy() / 2) + 0.5, (1, 2, 0))
        _ = viz.visualize_image_attr(attr_ig, original_image, method="blended_heat_map", sign="all",
                                     show_colorbar=True, title="Overlayed Integrated Gradients")


    def grad_cam(self, layer, example, guided=False):
        ...
        if guided:
            ...
            attr = gradcam.attribute(x_test, y_test[example])
        else:
            gradcam = LayerGradCam(self.model, self.model.conv2)
            attr = gradcam.attribute(x_test, y_test[example], relu_attributions=True)
        ...
        plt.imshow(org_image[example, :, :, :])
        plt.imshow(upsampled_attr.detach().numpy()[example, 0, :, :], cmap='hot', alpha=0.2)
        plt.show()