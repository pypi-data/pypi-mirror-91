...
from .models.enums import ListingSortType, ListingAssetType
...
class Zillow(Api):
    ...
    def download_listing_assets(
        ...
        asset_types: List[ListingAssetType] = [ListingAssetType.Image, ListingAssetType.Video],
        user_agent: Optional[Union[str, List[str]]] = None,
        proxy: Optional[Union[str, List[str]]] = None,
        ...
        if not asset_types:
            if self.debug:
                print('No asset types provided')
            
            return None, None

        def create_video(max_concurent_processes: Optional[int] = None, request_timeout_seconds: Optional[int] = None):
            if listing.video:
        ...
        if ListingAssetType.Image in asset_types:
            tasks.append(Task(create_video, max_concurent_processes=create_video_max_concurent_processes, request_timeout_seconds=request_timeout_seconds))

        if ListingAssetType.Video in asset_types:
            tasks.append(
                Task(self.__download_listing_images,
        ...
        return MultiProcess(tasks).solve()
    ...
    def __download_listing_video(
        ...
        res = self.download_async(urls_paths, user_agent=user_agent, proxy=proxy, max_request_try_count=max_request_try_count, debug=debug, max_concurent_processes=max_concurent_processes, request_timeout=request_timeout_seconds)
