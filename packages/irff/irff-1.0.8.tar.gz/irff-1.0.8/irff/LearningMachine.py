...
class LearningMachine(object):
  ...
  def __init__(self,initConfig='poscar.gen',
      ...
      self.get_ro(rodic)
      atoms             = self.c.ir.atoms
      if FreeAtoms is None:
         self.fixatoms  = None
      else:
         self.fixatoms  = FixAtoms(indices=[atom.index for atom in atoms if atom.index not in FreeAtoms])
  ...
  def mlmd(self,cwd,run_dir,atoms,Tmax,Iter,learnWay):        # run classic MD to test training results
      irmd = IRMD(atoms=atoms,label=self.label,
                  time_step=self.dt_mlmd,totstep=self.md_step,Tmax=Tmax,
