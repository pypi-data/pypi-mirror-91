...
class Ebus:
    ...
    async def async_read(self, msgdef, defaultprio=None, setprio=False, ttl=None):
        """
        Read Message.
        ...
        try:
            lines = tuple(
                [line async for line in self._async_request("read", msgdef.name, c=msgdef.circuit, p=prio, m=ttl)]
            )
    ...
    async def async_write(self, msgdef, value, ttl=0):
        ...
        # Write
        async for _ in self._async_request("write", msgdef.name, ";".join(values), c=msgdef.circuit, check=True):
            pass
    ...
    async def async_observe(self, msgdefs=None, defaultprio=None, setprio=False, ttl=None):
        """
        Observe.
        ...
        for msgdef in msgdefs:
            if msgdef.read:
                msg = await self.async_read(msgdef, defaultprio=defaultprio, setprio=setprio, ttl=ttl)
                if msg.valid:
                    msg = filter_msg(msg, msgdefs)
                if msg.valid:
                    data[msgdef] = msg
    ...
    async def _async_request(self, cmd, *args, infinite=False, check=False, **kwargs):
