...
class jsonfs:
    ...
    :MaxSize: maximum acceptable size of FsFile in MiB
    """    
    
    def __init__(self, FsFile:str="FS.json", MaxSize:int=32):
        self.FsJson = self.Internal.loadFS(self.Internal, FsFile, MaxSize)
        self.FsFileName = FsFile
        self.MaxSize = int(MaxSize * 1.049e+6)
    ...
    class Internal:
        ...
        def loadFS(self, FsFile:str, MaxSize:int):
            if not Path(FsFile).is_file():
                with open(FsFile, "w") as f:
                    ...
                    f.close()
            if Path(FsFile).stat().st_size > int(MaxSize * 1.049e+6): #if file is bigger than maxsize in MiB
        ...
        def saveFS(self, FsFile:str, MaxSize:int, FsJson):
            with open(FsFile, "w") as f:
                json.dumps(FsJson)
                f.close()
    ...
    def write(self, path:str, content):
        ...
        with open(self.FsFileName, "w") as f:
            f.write(json.dumps(self.FsJson))

    def remove(self, path:str):
        """
        Remove from the filesystem
        """
        self.FsJson["FS"].pop(base64.b64encode(path.encode()).decode())
        with open(self.FsFileName, "w") as f:
            f.write(json.dumps(self.FsJson))
