...
def test_get_auth_code(mocker, browser_redirect_flow, global_options, test_org):
    ...
    webbrowser.open.assert_called_once_with(
        f"https://auth.com/authorize?response_type=code&client_id={test_org['client_id']}&code_challenge_method=S256&code_challenge=test-challenge&redirect_uri=http://localhost:11001/callback&scope=admin offline_access&audience=https://api.com&state=foo"
    )
