...
class SalesforceDataSource(ToucanDataSource):
    query: str = Field(
        None,
        description='The SQL query to send',
        widget='sql',
    )
    ...
    endpoint: str = Field(None, description='The endpoint to query (without ending /)')
...
class SalesforceConnector(ToucanConnector):
    ...
    instance_url: str = Field(
        None,
        title='instance url',
        description='Baseroute URL of the salesforces instance to query (without ending /)',
    )
    ...
    def _retrieve_data(self, data_source: SalesforceDataSource) -> pd.DataFrame:
        ...
        return pd.DataFrame(
            self.generate_rows(
                session, data_source, endpoint=data_source.endpoint, params={'q': data_source.query}
            )
        )
