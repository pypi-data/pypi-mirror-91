...
def extract_images(rootdir):
    ...
    for idx, category in enumerate(files):
        for file in category:
            data.append([lookup[idx], _extract_images(file)])
...
def get_files(rootdir):
    ...
    for subfolder in subfolders:
        files.append(list_files(subfolder))
    return files
...
def extract_text(rootdir, ocr=False):
    ...
    if ocr is True:
        for idx, category in enumerate(files):
            for file in category:
                data.append([lookup[idx], _extract_text_ocr(file)])
    else:
        for idx, category in enumerate(files):
            for file in category:
                data.append([lookup[idx], _extract_text(file)])
    df = pd.DataFrame(data)
    return df
