...
def load_identity(data=None):
    ...
    if data:
        jf = {
            ...
            if k.lower() in ["client_id", "client_secret", "identity"]
        }
        return SimpleNamespace(**jf)

    if not os.path.isfile("identity.json"):
        raise FileExistsError(
            "A JSON file named `identity.json` must be accessible with your API credentials."
        )
...
def get_ga_data(data):
    ...
    if "ga_url" not in data:
        raise AttributeError(
            "You must provide the URL for your Google Analytics property."
        )
    ...
    try:
        print(
            "Pulling data from {} to {}.".format(data.ga_start_date, data.ga_end_date)
        )
        sessions = (
