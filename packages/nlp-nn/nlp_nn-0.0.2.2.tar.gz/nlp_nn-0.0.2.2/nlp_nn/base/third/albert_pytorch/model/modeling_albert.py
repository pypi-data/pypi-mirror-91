...
def load_tf_weights_in_albert(model, config, tf_checkpoint_path):
    """ Load tf checkpoints in a pytorch playground.
    """
    try:
        import re
        import numpy as np
        ...
        logging.error("Loading a TensorFlow playground in PyTorch, requires TensorFlow to be installed. Please see "
                      "https://www.tensorflow.org/install/ for installation instructions.")
        raise
    tf_path = os.path.abspath(tf_checkpoint_path)
    logging.info("Converting TensorFlow checkpoint from {}".format(tf_path))
    # Load weights from TF playground
    ...
    for name, array in zip(names, arrays):
        ...
        # which are not required for using pretrained playground
        if any(n in ["adam_v", "adam_m", "global_step"] for n in name):
            logging.info("Skipping {}".format("/".join(name)))
            continue
        pointer = model
        for m_name in name:
        ...
        try:
            ...
            raise
        logging.info("Initialize PyTorch weight {}".format(name))
...
class AlbertEmbeddings(nn.Module):
    ...
    def __init__(self, config):
        ...
        # self.LayerNorm is not snake-cased to stick with TensorFlow playground variable name and be able to load
...
class BertPooler(nn.Module):
    ...
    def forward(self, hidden_states):
        """ forward """
        # We "pool" the playground by simply taking the hidden state corresponding
        # to the first token.
...
class AlbertPreTrainedModel(PreTrainedModel):
    """ An abstract class to handle weights initialization and
        a simple interface for dowloading and loading pretrained third_models.
    """
...
BERT_START_DOCSTRING = r"""    The BERT playground was proposed in
    `BERT: Pre-training of Deep Bidirectional Transformers for Language Understanding`_
    by Jacob Devlin, Ming-Wei Chang, Kenton Lee and Kristina Toutanova. It's a bidirectional transformer
    pre-trained using a combination of masked language modeling objective and next sentence prediction
    on a large corpus comprising the Toronto Book Corpus and Wikipedia.
    This playground is a PyTorch `torch.nn.Module`_ sub-class. Use it as a regular PyTorch Module and
    ...
    Parameters:
        config (:class:`~pytorch_transformers.BertConfig`): Model configuration class with all the parameters of the playground. 
            Initializing with a config file does not load the weights associated with the playground, only the configuration.
            Check out the :meth:`~pytorch_transformers.PreTrainedModel.from_pretrained` method to load the playground weights.
"""
...
BERT_INPUTS_DOCSTRING = r"""
    Inputs:
        **input_ids**: ``torch.LongTensor`` of shape ``(batch_size, sequence_length)``:
            ...
            Bert is a playground with absolute position embeddings so it's usually advised to pad the inputs on
...
class AlbertModel(AlbertPreTrainedModel):
    ...
    Outputs: `Tuple` comprising various elements depending on the configuration (config) and inputs:
        **last_hidden_state**: ``torch.FloatTensor`` of shape ``(batch_size, sequence_length, hidden_size)``
            Sequence of hidden-states at the output of the last layer of the playground.
        ...
        **hidden_states**: (`optional`, returned when ``config.output_hidden_states=True``)
            ...
            Hidden-states of the playground at the output of each layer plus the initial embedding outputs.
    ...
    def _prune_heads(self, heads_to_prune):
        """ Prunes heads of the playground.
...
class AlbertForPreTraining(AlbertPreTrainedModel):
    ...
    Examples::
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        playground = BertForPreTraining.from_pretrained('bert-base-uncased')
        input_ids = torch.tensor(tokenizer.encode("Hello, my dog is cute")).unsqueeze(0)  # Batch size 1
        outputs = playground(input_ids)
...
class AlbertForMaskedLM(AlbertPreTrainedModel):
    ...
    Examples::
        ...
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        playground = BertForMaskedLM.from_pretrained('bert-base-uncased')
        input_ids = torch.tensor(tokenizer.encode("Hello, my dog is cute")).unsqueeze(0)  # Batch size 1
        outputs = playground(input_ids, masked_lm_labels=input_ids)
...
class AlbertForNextSentencePrediction(AlbertPreTrainedModel):
    ...
    Examples::
        ...
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        playground = BertForNextSentencePrediction.from_pretrained('bert-base-uncased')
        input_ids = torch.tensor(tokenizer.encode("Hello, my dog is cute")).unsqueeze(0)  # Batch size 1
        outputs = playground(input_ids)
...
class AlbertForSequenceClassification(AlbertPreTrainedModel):
    ...
    Examples::
        ...
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        playground = BertForSequenceClassification.from_pretrained('bert-base-uncased')
        input_ids = torch.tensor(tokenizer.encode("Hello, my dog is cute")).unsqueeze(0)  # Batch size 1
        labels = torch.tensor([1]).unsqueeze(0)  # Batch size 1
        outputs = playground(input_ids, labels=labels)
...
class AlbertForMultipleChoice(AlbertPreTrainedModel):
    ...
    Examples::
        ...
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        playground = BertForMultipleChoice.from_pretrained('bert-base-uncased')
...
class AlbertForTokenClassification(AlbertPreTrainedModel):
    ...
    Examples::
        ...
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        playground = BertForTokenClassification.from_pretrained('bert-base-uncased')
        input_ids = torch.tensor(tokenizer.encode("Hello, my dog is cute")).unsqueeze(0)  # Batch size 1
...
class AlbertForQuestionAnswering(AlbertPreTrainedModel):
    ...
    Examples::
        ...
        tokenizer = BertTokenizer.from_pretrained('bert-base-uncased')
        playground = BertForQuestionAnswering.from_pretrained('bert-base-uncased')
        input_ids = torch.tensor(tokenizer.encode("Hello, my dog is cute")).unsqueeze(0)  # Batch size 1
        start_positions = torch.tensor([1])
        end_positions = torch.tensor([3])
        outputs = playground(input_ids, start_positions=start_positions, end_positions=end_positions)
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, position_ids=None, head_mask=None,
        ...
        if start_positions is not None and end_positions is not None:
            ...
            # sometimes the start/end positions are outside our playground inputs, we ignore these terms
