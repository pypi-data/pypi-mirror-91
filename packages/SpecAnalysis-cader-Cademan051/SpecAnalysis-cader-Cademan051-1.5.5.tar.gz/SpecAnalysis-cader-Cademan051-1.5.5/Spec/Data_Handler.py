        ...
        if self.zoomAction.isChecked()==True:
            if QMouseEvent.button() == Qt.LeftButton:
                ...
                #Maximum y value in the select region
                    ...
                    self.y_start=(1001*self.Ymax-max(self.y)-1000*min(self.y))/(max(self.y)-min(self.y))
                    self.y_inc=(max(self.y)-min(self.y))/1001
                    ...
                    self.start=1000*((self.minx-self.x[0])/(self.x_avg))/m
                    
                    self.increment=(m*self.x_avg)/1000
...
Region between Channel """ + str(int(self.minx)) +" and Channel " + str(int(self.maxx))+ " selected")
    ...
    def PercentZoomIn(self):
        ...
        self.incrementZoomRegStorage.clear()
        self.incrementZoomRegStorage.append([midChan-reducedChanRangePerSide,midChan+reducedChanRangePerSide,self.Ymax])   
        
        
        if len(self.zoomRegStorage)==0:
            self.zoomRegStorage.append([midChan-reducedChanRangePerSide,midChan+reducedChanRangePerSide,self.Ymax,self.Ymax])   
        
        else:
          self.zoomRegStorage[-1]=[midChan-reducedChanRangePerSide,midChan+reducedChanRangePerSide,self.Ymax,self.Ymax]
...
Converted to Log Scale""")    
            ...
            if len(self.incrementZoomRegStorage)>=1:
                ...
                Ymax=self.incrementZoomRegStorage[-1][2]
                ...
                if min(self.y)>0:
                    ...
                    y_min=1
                y_start=int(round(((1001*Ymax-y_max-1000*y_min)/(y_max-y_min))))
                ...
                if self.incrementZoomRegStorage[-1][0]<self.incrementZoomRegStorage[-1][1]:
                ...
                elif self.incrementZoomRegStorage[-1][0]>self.incrementZoomRegStorage[-1][1]:
                    self.plt.setRange(xRange=[self.incrementZoomRegStorage[-1][1],self.incrementZoomRegStorage[-1][0]])
                    ...
                    start=1000*((minx-self.x[0])/(x_avg))/m
                
                
                
                    self.s2.setValue(start)
                    
                    self.plt.setRange(xRange=[self.incrementZoomRegStorage[-1][0],self.incrementZoomRegStorage[-1][1]])
                    
                    self.s2.setValue(self.start)
                 
                    
                elif self.incrementZoomRegStorage[-1][0]==self.incrementZoomRegStorage[-1][1]:    
                    self.plt.setRange(xRange=[self.incrementZoomRegStorage[-1][1],self.incrementZoomRegStorage[-1][0]+1])
...
Converted to Linear Scale""")
            ...
            if len(self.incrementZoomRegStorage)>=1:
                self.plt.setRange(yRange=[min(self.y),self.incrementZoomRegStorage[-1][2]])
