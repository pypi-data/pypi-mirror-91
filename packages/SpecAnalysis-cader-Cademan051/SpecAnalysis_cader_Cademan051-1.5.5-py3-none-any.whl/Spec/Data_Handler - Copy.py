...
class Spectrum(QMainWindow):
    ...
    def __init__(self):
        ...
        #Monte Carlo summation method
        self.mcSumAction=QAction(QIcon(icondir+"MCIcon.png"),"Monte Carlo Sum",self,checkable=True)
        self.mcSumAction.triggered.connect(self.MonteCarloSum)
        ...
        self.probAction=QAction(QIcon(icondir+"probIcon.png"),"Probability",self,checkable=True)
        self.probAction.triggered.connect(self.Prob)
        ...
        toolBar.addAction(self.mcSumAction)
        toolBar.addAction(self.mcmcAction)
        toolBar.addAction(self.probAction)
        ...
        self.dataWidget.setMaximumHeight(150)
        self.dataWidget.setMaximumWidth(2100)
        ...
        self.grid.addWidget(self.dataWidget,3,1)
        ...
        self.grid.addWidget(self.cursor_label,0,1)
    ...
    def About(self):
        ...
        about.setText("Welcome! This is a program designed by Cade Rodgers for the purpose of analyzing spectra.")
        about.exec()
    ...
    def Refresh(self):
        ...
        self.mcSumAction.setEnabled(True)
        ...
        if self.yScale.isChecked()==True:
                self.plt.setRange(yRange=[0,self.Ymax])
        ...
        if self.mcSumAction.isChecked()==True:
            self.plt.removeItem(self.reg1fill)
            self.plt.removeItem(self.reg2fill)
            self.plt.removeItem(self.regplot)
            self.plt.removeItem(self.totalfill)
    ...
    #Unchecks many of the tabs
        ...
        self.mcSumAction.setChecked(False)
    ...
    def ZoomOut(self):
        ...
        if len(self.zoomRegStorage)==0:
        ...
        elif len(self.zoomRegStorage)==1:
            self.plt.setRange(xRange=[self.x[0],self.x[-1]])
            ...
            if self.logScale.isChecked()==True:
                ...
                self.plt.setRange(yRange=[np.log10(logMinY),np.log10(np.max(self.y))])
            else:
                self.plt.setRange(yRange=[np.amin(self.y),np.max(self.y)])
        ...
        if len(self.zoomRegStorage)>1:
            self.zoomRegStorage.pop()
            oldVals=self.zoomRegStorage[-1]
            ...
            self.y_start=(100001*self.Ymax-max(self.y)-100000*min(self.y))/(max(self.y)-min(self.y))
            self.y_inc=(max(self.y)-min(self.y))/100001
            ...
            self.x_avg=oldVals[1]-oldVals[0]
                    
                    
            m=(self.x[-1]-self.x[0]-self.x_avg)/self.x_avg
            self.start=10000*(oldVals[0]/(self.x_avg))/m
            ...
            self.plt.setRange(xRange=[oldVals[0],oldVals[1]])
            
            
            if self.logScale.isChecked()==False:
                
             
                if self.s1.value()==100000:
                    self.plt.setRange(yRange=[np.amin(self.y),max(self.y)])
                    
                else:
                    self.plt.setRange(yRange=[np.amin(self.y),int(oldVals[2])])
            ...
            if self.logScale.isChecked()==True:
                ...
                if min(self.y)>0:
                    min_y=min(self.y)
                if self.s1.value()==100000:
                    self.plt.setRange(yRange=[np.log10(np.log10(min_y)),np.log10(max(self.y))])
                else:
                    
                    self.plt.setRange(yRange=[np.log10(min_y),np.log10(int(oldVals[2]))])
        ...
        if self.zoomAction.isChecked()==True:
            if QMouseEvent.button() == Qt.LeftButton:
                ...
                if self.zoomClickCounter==2:
                    ...
                    self.grid.addWidget(self.s2,2,1)
                ...
                #Takes input from SelectRegion method to set the plot's range
                    ...
                    self.plt.setRange(yRange=[np.amin(self.y),self.Ymax])
...
Region between Channel """ + str(int(self.minx)) +" and Channel " + str(int(self.maxx))+ " selected")
                    ...
                    if self.logScale.isChecked()==True:
                        if min(self.y)>0:
                            self.plt.setRange(yRange=[np.log10(np.amin(self.y)),np.log10(self.Ymax)])
    ...
    def sliderReleased(self):
        ...
        if self.pressed==True:
            ...
            self.zoomRegStorage[-1][0]+=xChange
            self.zoomRegStorage[-1][1]+=xChange
            
            if self.endSlideVal==0:
                offset=self.zoomRegStorage[-1][0]-self.x[0]
                
                self.zoomRegStorage[-1][0]-=offset
                self.zoomRegStorage[-1][1]-=offset
            
            if self.endSlideVal==10000:
                offset=self.x[-1]-self.zoomRegStorage[-1][1]
    ...
    def Move_Y(self, value):
        ...
        if len(self.zoomRegStorage)>=1:
            ...
            if self.logScale.isChecked()==True:
                ...
                if min(self.y)>0:
                    self.plt.setRange(yRange=[np.log10(np.amin(self.y)),np.log10(self.Ymax+y)])
                ...
                self.plt.setRange(yRange=[min(self.y),self.Ymax+y])
 ...
 #Sets y scale to the maximum y=value of the spectrum
        ...
        if self.logScale.isChecked()==True:
            ...
            if min(self.y)<=0:
                ...
                min_y=np.log10(1)
            if min(self.y)>0:
                min_y=np.log10(min(self.y))
       
        self.plt.setRange(yRange=[min_y,maximum_y])
      
        self.s1.setValue(100000)
        self.dataWidget.append(
"""
...
Converted to Log Scale""")    
            ...
            if len(self.zoomRegStorage)>=1:
                ...
                if min(self.y)>0:
                    ...
                    y_min=1
                y_start=int(round(((100001*Ymax-y_max-100000*y_min)/(y_max-y_min))))
                
            
                    
                self.s1.setValue(y_start)
                ...
                if self.zoomRegStorage[-1][0]<self.zoomRegStorage[-1][1]:
                    
                    minx=self.zoomRegStorage[-1][0]
                    maxx=self.zoomRegStorage[-1][1]
                    
                   
                   
                    self.plt.setRange(xRange=[minx,maxx])
                
                    
                elif self.zoomRegStorage[-1][0]>self.zoomRegStorage[-1][1]:
                    self.plt.setRange(xRange=[self.zoomRegStorage[-1][1],self.zoomRegStorage[-1][0]])
                    ...
                    m=(self.x[-1]-self.x[0]-x_avg)/x_avg
                    
                    start=10000*((minx-self.x[0])/(x_avg))/m
                
                
                
                    self.s2.setValue(start)
                    
                    self.plt.setRange(xRange=[self.zoomRegStorage[-1][0],self.zoomRegStorage[-1][1]])
                    
                    self.s2.setValue(self.start)
                 
                    
                elif self.zoomRegStorage[-1][0]==self.zoomRegStorage[-1][1]:    
                    self.plt.setRange(xRange=[self.zoomRegStorage[-1][1],self.zoomRegStorage[-1][0]+1])
            ...
            if self.sumAction.isChecked()==True or self.mcmcAction.isChecked()==True or self.mcSumAction.isChecked()==True:
                y=[]
...
Converted to Linear Scale""")
            ...
            if len(self.zoomRegStorage)>=1:
                self.plt.setRange(yRange=[min(self.y),self.zoomRegStorage[-1][2]])
                ...
                if self.zoomRegStorage[-1][0]<self.zoomRegStorage[-1][1]:
                    ...
                    self.s2.setValue(self.x_value+1)
                    self.s2.setValue(self.x_value)
    ...
    def peakCursor(self):
        if self.newfile!="":
            ...
            if self.peakSelect.isChecked()==True:
                ...
                if self.sumAction.isChecked()==True or self.mcSumAction.isChecked()==True or self.mcmcAction.isChecked()==True:
                    self.mcSumAction.setEnabled(True)
                    self.mcmcAction.setEnabled(True)
                    self.gaussFitAction.setEnabled(True)
                    self.sumAction.setEnabled(True)
...
Select Peak first""")
    ...
    def Sum(self,PeakSelect):
        if self.sumAction.isChecked()==True:
            ...
            self.mcSumAction.setEnabled(False)
...
Select peak and background first!""")   
            ...
            if self.backSelec.isChecked()==True:
                ...
                self.checked=False
                self.minWidget=QDialog()
                checkbox=QCheckBox("",self)
                checkbox.stateChanged.connect(self.Checkbox)
                minVal1=QLineEdit(self.minWidget)
                minVal2=QLineEdit(self.minWidget)
                
                self.minVal1=minVal1
                self.minVal2=minVal2
                sampleSize=QLineEdit(self.minWidget)
                setSeed=QLineEdit(self.minWidget)
                buttonBox= QDialogButtonBox(QDialogButtonBox.Ok, self)
                layout= QFormLayout(self.minWidget)
                layout.addRow("Randomize sub-region width",checkbox)
                layout.addRow("Sub-region width of region 1: ", minVal1)
                layout.addRow("Sub-region width of region 2: ", minVal2)
                layout.addRow("Number of samples: ", sampleSize)
                layout.addRow("Set seed: ",setSeed)
                layout.addWidget(buttonBox)
                buttonBox.accepted.connect(self.minWidget.accept)
                ...
                if minVal1.text()=="":
                    ...
                    minimumWidth1=int(minVal1.text())
                    
                if minVal2.text()=="":
                    minimumWidth2=5
                else:
                    minimumWidth2=int(minVal2.text())
                ...
                if setSeed.text()=="":
                    ...
                    seed=int(setSeed.text())
                    
                
                
                x1_first_channel=self.x1range[0]  
                x1_last_channel=self.x1range[-1]
                    
                x2_first_channel=self.x2range[0]  
                x2_last_channel=self.x2range[-1]
                ...
                if minimumWidth1>=abs(x1_last_channel-x1_first_channel):
                    minimumWidth1=0
                
                if minimumWidth2>=abs(x2_last_channel-x2_first_channel):
                    minimumWidth2=0  
                ...
                for i in range(samples):
                    if self.checked==False:
                        rand_channel_11=np.random.randint(x1_first_channel,(x1_last_channel-minimumWidth1)+1)
                    else:
                        rand_channel_11=np.random.randint(x1_first_channel,x1_last_channel+1)
                        rand_channel_12=np.random.randint(x1_first_channel,x1_last_channel+1)
                        
                    if self.checked==False:
                        rand_channel_21=np.random.randint(x2_first_channel,(x2_last_channel-minimumWidth2)+1)
    
                    else:
                        rand_channel_21=np.random.randint(x2_first_channel,x2_last_channel+1)
                        rand_channel_22=np.random.randint(x2_first_channel,x2_last_channel+1)

                        
                    if self.checked==True:
                        if rand_channel_11>=rand_channel_12:
                            stop1=rand_channel_11
                            start1=rand_channel_12
                        else:
                            stop1=rand_channel_12
                            start1=rand_channel_11
                            
                        if rand_channel_21>=rand_channel_22:
                            stop2=rand_channel_21
                            start2=rand_channel_22
                        else:
                            stop2=rand_channel_21
                            start2=rand_channel_21
                    if self.checked==False:
                        start1=rand_channel_11
                        stop1=rand_channel_11+minimumWidth1
                        
                        
                        start2=rand_channel_21
                        stop2=rand_channel_21+minimumWidth2
                        
                    newbkg1=self.y[int(round(start1-self.x[0])):int(round((stop1+1)-self.x[0]))] 
                    newbkg2=self.y[int(round(start2-self.x[0])):int(round((stop2+1)-self.x[0]))]
                        
                    bkgavg1=sum(newbkg1)/len(newbkg1)
                    bkgavg2=sum(newbkg2)/len(newbkg2)
                        
                    
                        
                    x1_mean=(stop1+start1)/2
                    x2_mean=(stop2+start2)/2
                        
                    newslope=(bkgavg2-bkgavg1)/(x2_mean-x1_mean)
                    ...
                    for i in range(len(self.peakx)):
                        x=self.peakx[i]-x1_mean
                        b=bkgavg1+(newslope*x)
                        newbkg.append(float(b))

                    total_vals=np.array(self.peaky)
                    bkg_vals=np.array(newbkg)
                        
                    net_counts=total_vals-bkg_vals
                        
                        
                    netsum=sum(net_counts)
                    
                    if self.checked==True:
                        if abs(stop1-start1)<minimumWidth1:
                            continue
                    
                        if abs(stop2-start2)<minimumWidth2:
                            continue
                    ...
                    net_sums.append(float(netsum))

                        
                    sum_unc=(np.sum(total_vals)+np.sum(bkg_vals))**(1/2)
                    sum_uncs.append(sum_unc)
                        
                    nonNegCounts=[count if count>=0 else 0 for count in net_counts]
                    
                    centroid=np.multiply(self.peakx,nonNegCounts)
                    centroid=sum(centroid)/sum(nonNegCounts)
                    centroids.append(centroid)
                    
                    
                    new_x_squared=(self.peakx-centroid)**2
                    new_xy=np.multiply(new_x_squared,nonNegCounts)
                    sum_xy=np.sum(new_xy)
                    N=np.sum(nonNegCounts)
                    N_1=N-1
                    standev=sum_xy/N_1
                    standev=standev
                    standev=(standev)**(1/2)

                    new_standev=standev/((N**(1/2)))
                        
                    cent_errs.append(new_standev)
                    
                    
                for i in range(len(net_sums)):
                    if np.isfinite(net_sums[i])==False:
                        net_sums[i]=0
                    
                    if np.isfinite(sum_uncs[i])==False:
                        sum_uncs[i]=0
                        
                    if np.isfinite(centroids[i])==False:
                        centroids[i]=0
                        
                    if np.isfinite(cent_errs[i])==False:
                        cent_errs[i]=0


                m3_sum_median=np.median(net_sums)
                m3_sum_unc=np.median(sum_uncs)
                    
                m3_centroid=np.median(centroids)
                m3_cent_unc=np.median(cent_errs)
                ...
                sumplt=pg.PlotWidget()
                
                        
                countmin,countmax=np.amin(net_sums),np.amax(net_sums)
                x_smooth1=np.linspace(countmin,countmax,200)
                y_smooth1=sp.stats.gaussian_kde(net_sums)(x_smooth1)
                pen=pg.mkPen(color="k",width=2)
                sumplt.plot(x_smooth1,y_smooth1,pen=pen,antialias=True)
                sumplt.setMouseEnabled(x=False,y=False)
                
                self.sum_tab=sumplt
                self.tabs.addTab(self.sum_tab,"Net Count Distribution")
                
                
                sum_unc_plt=pg.PlotWidget()
                countmin,countmax=np.amin(sum_uncs),np.amax(sum_uncs)
                x_smooth2=np.linspace(countmin,countmax,200) 
                y_smooth2=sp.stats.gaussian_kde(sum_uncs)(x_smooth2)
                pen=pg.mkPen(color="k",width=2)
                sum_unc_plt.plot(x_smooth2,y_smooth2,pen=pen,antialias=True)
                sum_unc_plt.setMouseEnabled(x=False,y=False)
                
                self.sum_unc_tab=sum_unc_plt
                self.tabs.addTab(self.sum_unc_tab,"Net Count Uncertainty Distribution")
                
                
                cent_plt=pg.PlotWidget()
                countmin,countmax=np.amin(centroids),np.amax(centroids)
                x_smooth3=np.linspace(countmin,countmax,200)
                y_smooth3=sp.stats.gaussian_kde(centroids)(x_smooth3)
                pen=pg.mkPen(color="k",width=2)
                cent_plt.plot(x_smooth3,y_smooth3,pen=pen,antialias=True)
                cent_plt.setMouseEnabled(x=False,y=False)
                
                self.cent_tab=cent_plt
                self.tabs.addTab(self.cent_tab,"Centroid Distribution")
                
                cent_unc_plt=pg.PlotWidget()
                countmin,countmax=np.amin(cent_errs),np.amax(cent_errs)
                x_smooth4=np.linspace(countmin,countmax,200)
                y_smooth4=sp.stats.gaussian_kde(cent_errs)(x_smooth4)
                pen=pg.mkPen(color="k",width=2)
                cent_unc_plt.plot(x_smooth4,y_smooth4,pen=pen,antialias=True)
                cent_unc_plt.setMouseEnabled(x=False,y=False)
                
                self.cent_unc_tab=cent_unc_plt
                self.tabs.addTab(self.cent_unc_tab,"Centroid Uncertainty Distribution")
                self.peakSelect.setChecked(False)
                self.backSelec.setChecked(False)
...
Peak Channel Range= """+str(firstchannel)+" to " + str(lastchannel)+
"""
Signal Counts= """ +str(m3_sum_median)+ " +/- " +str(m3_sum_unc)+
"""
Centroid= """+ str(m3_centroid)+ " +/- " +str(m3_cent_unc))
                self.dataWidget.moveCursor(QtGui.QTextCursor.End)
     
        


        if self.mcSumAction.isChecked()==False:
    ...
    def Checkbox(self,state):
        ...
        if state==QtCore.Qt.Checked:
            self.checked=True
            self.minVal1.setEnabled(False)
            self.minVal2.setEnabled(False)
            
        else:
            self.checked=False
            self.minVal1.setEnabled(True)
            self.minVal2.setEnabled(True)
...
#Having NaN values, or 0, as the background raises an error when running the Poisson
#This loop replaces any zeroes and NaN values with a small number         
...
"""
                    ...
                    trace1=np.array(bayesTrunc(peakXRange,peakCounts,backCounts,tot_width,bkg_width,samples,tune,chain_num))
                    trace1=np.transpose(trace1)
                    trace1=trace1[0]
                    
                
                    
                    bayesGam=robjects.r(bayesianMCMCG)
                    trace2=np.array(bayesGam(peakXRange,peakCounts,backCounts,tot_width,bkg_width,samples,tune,chain_num))
...
BAYESIAN ANALYSIS"""+
"""
Parameters: Sample Size= """+str(samples)+"\t Burn= "+str(tune)+"    Chains= "+str(chain_num)+

"""
Peak Channel Range= """+str(firstchannel)+" to " + str(lastchannel)+
                                   
"""
Signal Counts (Trunc. Norm.)= """ +str(Median_S)+ " + "+ str(Upper_Bound)+ "/- "+str(Lower_Bound) +
"""
Signal Counts (Gamma)= """ +str(g_Median_S)+ " + "+ str(g_Upper_Bound)+ "/- "+str(g_Lower_Bound) +

"""
Centroid (Trunc. Norm.)= """ + str(t_x_median)+  " + "  + str(t_upper)+"/- " + str(t_lower)+
"""
Centroid (Gamma)= """ + str(g_x_median)+  " + "  + str(g_upper)+"/- " + str(g_lower)+
"""
Signal Counts Percentiles (Trunc. Norm.): """+
"""
2.5%= """+str(x_2)+ "   16%= "+str(x_16)+"   50%= "+str(x_50)+"   84%= "+str(x_84)+"   97.5%= "+str(x_97)+
"""
Signal Counts Percentiles (Gamma): """+
"""
2.5%= """+str(g_x_2)+ "   16%= "+str(g_x_16)+"   50%= "+str(g_x_50)+"   84%= "+str(g_x_84)+"   97.5%= "+str(g_x_97))
    ...
    def Prob(self):
        if self.probAction.isChecked()==True:
            if self.mcmcAction.isChecked()==False:
                self.dataWidget.append(
"""
Run MCMC first!""")  
                ...
                self.probAction.setChecked(False)
            ...
            if self.mcmcAction.isChecked()==True:
                ...
                title=QLabel(probWidget)
                prob_x1 = QLineEdit(probWidget)
                prob_x2=QLineEdit(probWidget)
        
                buttonBox = QDialogButtonBox(QDialogButtonBox.Ok, self)
                layout = QFormLayout(probWidget)
                layout.addRow("Calculate Probability of Signal Between:",title)
                layout.addRow("Value 1 ", prob_x1)
                layout.addRow("Value 2 ",prob_x2)
                layout.addRow("Trunc. Norm.",self.b1)
                layout.addRow("Gamma",self.b2)
    
                layout.addWidget(buttonBox)
                buttonBox.accepted.connect(probWidget.accept)
                probWidget.exec()
                
                
                x1=float(prob_x1.text())
                x2=float(prob_x2.text())
                ...
                probability=abs(probability2-probability1) 
                
                if self.tnProb==True:
                    self.dataWidget.append(
"""
Probability of the signal being between """+str(x1)+" and "+str(x2)+ " counts is "+str(probability)+"% (Trunc. Norm.)")
                if self.gProb==True:
                    self.dataWidget.append(
"""
Probability of the signal being between """+str(x1)+" and "+str(x2)+ " counts is "+str(probability)+"% (Gamma)")
...
Select peak and background first!""")
            ...
            if self.backSelec.isChecked()==True:
                ...
                if self.sigmaP==True:
                    mean=0
                    std=1000
                    wEst=(self.peakx[-1]-self.peakx[0])/6
...
GAUSSIAN FIT"""+
"""
Parameters: Sample Size= """+str(samples)+"\t Burn= "+str(tune)+"     Chains= "+str(chainNum)+
"""
Peak Channel Range= """+str(int(self.peakx[0]))+" to " + str(int(self.peakx[-1]) )+
"""
a= """+str(heightPers[2])+" + "+str(heightPers[3]-heightPers[2])+"/- "+str(heightPers[2]-heightPers[1])+
"""
b= """+str(centroid)+" + "+str(upperC)+"/- "+str(lowerC)+
"""
c= """+str(sigmaVals[2])+" + "+str(sigmaVals[3]-sigmaVals[2])+"/- "+str(sigmaVals[2]-sigmaVals[1])+
"""
Signal Counts= """+str(sigPers[2])+" + "+str(sigPers[3]-sigPers[2])+"/- "+str(sigPers[2]-sigPers[1]))
                
                self.dataWidget.moveCursor(QtGui.QTextCursor.End)                       
   
                thin=int(len(trace[0])/1500)
            
                
            #When log scale is one, it takes tremendously longer to generate the 
            #Gaussian plots for some reason
                ...
                for i in range(0,len(trace[0]),thin):
                    ...
                    for x in x_vals:
                        y=height * np.exp(-((x - cent)**2 / (2 * width**2)))
                
                        background=trace[3][i]*x+trace[4][i]
                
                        gaussY=y+background
                        y_vals.append(gaussY)
