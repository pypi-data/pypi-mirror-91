...
class Sso:
    ...
    def fast_api_sso(self, func):
        ...
        def fast_api(*args, **kwargs):
            ...
            """
            token = kwargs.get(self._token_key)
            if token and self._rd.sismember(self._token_key, token):
                pass
            else:
    ...
    def django_sso(self, func):
        ...
        def django(request):
            ...
            """
            token = request.META.get("HTTP_" + self._token_key.upper())
            if token and self._rd.sismember(self._token_key, token):
                pass
            else:
                # 返回失败
    ...
    async def _add_process_time_header(self, request: Request, call_next):
        ...
        """
        token = request.headers.get(self._token_key)
        # print(token)
        if token and self._rd.sismember(self._token_key, token):
            pass
        else:
