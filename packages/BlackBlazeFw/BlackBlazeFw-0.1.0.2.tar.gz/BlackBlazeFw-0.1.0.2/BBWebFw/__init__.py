...
class webApp:
    def __init__(self, name, server, hostAddr="0.0.0.0:8000"):
        self.urls = {}
        self.server = server
        self.name = name
    ...
    def run(self, app, debug=False, host=self.hostAddr):
        backend.api.run(self, app)

    def handle_request(self, request, mimetype_):
        return backend.api.handle_request(self, request,  mimetype=mimetype_)
