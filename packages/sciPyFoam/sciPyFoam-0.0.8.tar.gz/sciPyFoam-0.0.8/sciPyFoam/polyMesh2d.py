...
def plotMeshTopology(ax,caseDir,ind_cell=None,index_intFace=None):
    caseDir=os.path.abspath(caseDir)
    internalMesh = '%s/VTK/%s_0.vtk'%(caseDir,caseDir.split('/')[-1])
    if(not os.path.exists(internalMesh)):
    ...
    ax.fill(x[cells_rect[ind_cell]], y[cells_rect[ind_cell]], label='The %d$_{th}$ cell'%(ind_cell))
    ...
    for face in faces_own_to_cell:
        ...
        ax.plot(x_face,y_face, color='r', lw=3)
        ...
        len_arrow=np.max([x_face.max()-x_face.min(),y_face.max()-y_face.min()])/3
        ax.annotate("", xy=xy_cf-norm*len_arrow/2, xytext=xy_cf+norm*len_arrow/2,arrowprops=dict(arrowstyle="->"))
    ...
    for face in faces_neighbour_to_cell:
        ...
        x_face,y_face,z_face=x[faces['index'][face]], y[faces['index'][face]], z[faces['index'][face]]
        ax.plot(x_face,y_face, color='b', lw=3)
        # plot the normal vector of the face
        norm=np.cross([x_face[1] - x_face[0], y_face[1] - y_face[0], z_face[1] - z_face[0]],
                     [x_face[2] - x_face[1], y_face[2] - y_face[1], z_face[2] - z_face[1]])
        norm=norm[0:2]
