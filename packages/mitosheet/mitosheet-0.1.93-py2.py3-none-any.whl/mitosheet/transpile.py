...
def get_previous_formula_step(steps, step_idx):
    """
    Returns the most recent formula step before the step with step_idx. 

    Returns None if this is the first formula step!
    """
    for step in reversed(steps[:step_idx]):
        if step['step_type'] == 'formula':
            return step
    
    return None

def formula_step_changes_formulas_for_sheet(steps, formula_step, formula_step_idx, sheet_index):
    """
    ...
    """
    previous_step = get_previous_formula_step(steps, formula_step_idx)
...
def get_formula_step_code(
    ...
    for sheet_index in range(len(formula_step['column_evaluation_graph'])):
        ...
        if not formula_step_changes_formulas_for_sheet(steps, formula_step, formula_step_idx, sheet_index):
            continue
