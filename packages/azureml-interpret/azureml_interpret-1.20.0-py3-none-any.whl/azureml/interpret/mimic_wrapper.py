...
from azureml.interpret.common.exceptions import ConflictingRawTransformationsException, \
    ...
    OptionalDependencyMissingException, ScenarioNotSupportedException
...
class MimicWrapper(ChainedIdentity):
    ...
    def _get_surrogate_model_replication_measure(self, training_data):
        ...
        surrogate_model_predictions = self._internal_explainer.surrogate_model.predict(training_data)
        teacher_model_predictions = self._teacher_model.predict(training_data)

        if self._internal_explainer.classes is not None:
            if len(self._internal_explainer.classes) > 2:
                replication_measure = accuracy_score(teacher_model_predictions, surrogate_model_predictions)
            else:
                raise ScenarioNotSupportedException("Replication measure is not supported for binary classification")
        else:
            if training_data.shape[0] == 1:
                raise ScenarioNotSupportedException("Replication measure for regression surrogate not supported "
