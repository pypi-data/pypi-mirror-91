...
class InferenceModel(BaseModel):
            ...
            batch_predictions: An iterable that produces prediction dicts.
                Each prediction dict MUST contain the following keys:
                    ...
                    * features (Dict[str, FieldValue]): Values for all the features
                        in the prediction
                    * prediction (Dict[str, FieldValue]): Prediction result
    ...
    @validate_model_ready
    def log_outputs(self, id: str, outputs: Dict[str, FieldValue]):
        """Logs outputs of a single prediction.

        Args:
            id: Prediction identifier.
            outputs: Final outputs of the prediction.
        """
    ...
    def log_batch_outputs(self, batch_outputs: Iterable[dict]):
        ...
        Args:
            batch_outputs: An iterable that produces outputs dicts.
                Each dict MUST contain the following keys:
                    * id (str): Prediction identifier.
                    * outputs (Dict[str, FieldValue]): Final outputs of the prediction.
        """
        self._log_batch_inference(
            batch=batch_outputs,
