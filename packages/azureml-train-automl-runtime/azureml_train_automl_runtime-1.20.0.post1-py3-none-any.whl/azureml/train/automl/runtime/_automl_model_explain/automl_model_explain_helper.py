...
from azureml.interpret.common.exceptions import OptionalDependencyMissingException, ScenarioNotSupportedException
...
def _log_blackbox_explainer_replication_measure(explainer: MimicWrapper, training_data: DataInputType,
    ...
    try:
        replication_score = explainer._get_surrogate_model_replication_measure(training_data=training_data)
        logger.info("The replication score for teacher model {0} with surrogate model {1} is:- {2}".format(
            run_algorithm_name,
            _get_user_friendly_surrogate_model_name(explainer._internal_explainer.surrogate_model),
            replication_score))
    except Exception as e:
        if isinstance(e, ScenarioNotSupportedException) or isinstance(e, OptionalDependencyMissingException):
            error_str = e.message
        else:
            error_str = None
        logger.info("The replication score for teacher model {0} with surrogate model {1} could not be found. "
                    "Reason {2}".format(run_algorithm_name,
                                        _get_user_friendly_surrogate_model_name(
                                            explainer._internal_explainer.surrogate_model),
                                        error_str))
