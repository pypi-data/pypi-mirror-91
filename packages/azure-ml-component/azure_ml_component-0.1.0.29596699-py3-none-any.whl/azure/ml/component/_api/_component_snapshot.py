...
class LocalComponentSnapshot(ComponentSnapshot):
    ...
    def _create_snapshot_in_folder(self, folder, snapshot_items_iterator) -> Path:
        ...
        for snapshot_item in snapshot_items_iterator:
            if snapshot_item.absolute_path.is_file():
                target_path = os.path.join(folder, snapshot_item.relative_path)
                self.logger.info('\t{} -> {}'.format(snapshot_item.absolute_path, target_path))
                copy2(snapshot_item.absolute_path, target_path)
            else:
                os.mkdir(os.path.join(folder, snapshot_item.relative_path))

        self.logger.info('\n')
        self.logger.info('Successfully built snapshot:{}'.format(folder.as_posix()))
        return folder
