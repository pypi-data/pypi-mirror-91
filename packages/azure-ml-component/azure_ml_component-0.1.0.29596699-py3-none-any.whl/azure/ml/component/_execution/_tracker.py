...
class RunHistoryTracker:
        ...
        if track_run_history:
            ...
            if run:
                print_to_terminal(
                    ...
                    'of component [ %s ] to record execution infomation.' %
                    component.name)
            else:
                print_to_terminal(
    ...
    def without_definition(cls, workspace, experiment_name, track_run_history, path=None):
        ...
        finished_msg = 'Finished initialize run history tracker of experiment [ %s ] to record execution ' \
                       'infomation.' % experiment_name
        if track_run_history:
            run = get_current_run()
            if run:
                print_to_terminal(
                    'A run exists in current environment, will reuse it to initialize '
                    'run history tracker of pipeline to record execution infomation.')
                finished_msg = 'Finished initialize run history tracker of pipeline to record execution infomation.'
            else:
                print_to_terminal(
