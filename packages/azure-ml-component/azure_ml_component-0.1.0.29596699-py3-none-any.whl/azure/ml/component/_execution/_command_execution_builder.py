...
from .._core._component_definition import ComponentType, CommandComponentDefinition, ParallelComponentDefinition
...
class ComponentRunCommand:
    ...
    def __init__(self, component, working_dir, os_type='Linux', python_path='python'):
        self.component = component
        self.working_dir = working_dir
    ...
    def generate_command(self, use_docker, check_data_exist=True, container_input_prefix=CONTAINER_INPUT_PATH,
        ...
        command = self.generate_component_command(self.component._definition, inputs, output_path, self.python_path)
    ...
    def generate_outputs_in_docker(self, default_output_path, container_output_prefix=CONTAINER_OUTPUT_PATH):
        default_container_output_prefix = container_output_prefix + '/default'
        volumes = {str(default_output_path): {'bind': default_container_output_prefix, 'mode': 'rw'}}
        ...
        for output_port_name, value in self.component.outputs.items():
            ...
            if value._path_on_compute:
                ...
                volumes[str(value._path_on_compute)] = {'bind': output_port_path, 'mode': 'rw'}
    ...
    def generate_component_command(
        cls, definition: CommandComponentDefinition, inputs=None, outputs=None, python_path='python',
    ):
        if definition.type in {ComponentType.CommandComponent, ComponentType.MPIComponent}:
            ...
            if definition._is_any_command():
                return cls.resolve_command(definition.command, inputs, outputs, split=False)
            else:
                command = cls.resolve_command(definition.command, inputs, outputs, split=True)
                command[0] = python_path
                return command
        elif definition.type == ComponentType.ParallelComponent:
            return cls.generate_parallel_command(definition, inputs, outputs, python_path)
        else:
    ...
    def generate_parallel_command(
        cls, definition: ParallelComponentDefinition, inputs=None, outputs=None, python_path='python',
    ):
        ...
        command = [python_path, MOCK_PARALLEL_DRIVER, '--scoring_module_name', definition.entry]
        input_index = 0
