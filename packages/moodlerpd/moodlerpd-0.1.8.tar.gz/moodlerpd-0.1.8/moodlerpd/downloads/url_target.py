...
class URLTarget(object):
    ...
    def __init__(
        ...
        # get filename
        self.filename = self.file.content_filename
    ...
    @staticmethod
    def _generate_path_of_non_existent_file(wish_path: str) -> str:
    ...
    def _rename_if_exists(self, path: str) -> str:
        ...
        if self.options.get('download_generate_path', True):
            path = self._generate_path_of_non_existent_file(path)

        logging.debug('T%s - Setting up target file: "%s"', self.thread_id, path)
        try:
            open(path, 'x').close()
        except Exception as e:
            self.fs_lock.release()
            logging.error('T%s - Failed setting up target file: "%s"', self.thread_id, path)
            raise e
    ...
    def try_rename_old_file(self) -> bool:
        ...
        if self.options.get('download_generate_path', True):
            new_path = self._generate_path_of_non_existent_file(new_path)
    ...
    def set_path(self, ignore_attributes: bool = False):
        ...
        valid_filename = PathTools.to_valid_name(file_name)

        if self.file.content_type == 'description' and not ignore_attributes:
            self.file.saved_to = str(Path(self.destination) / (valid_filename + '.md'))
            ...
            self.file.saved_to = str(Path(self.destination) / (valid_filename + '.desktop'))
            if os.name == 'nt' or platform.system() == "Darwin":
                self.file.saved_to = str(Path(self.destination) / (valid_filename + '.URL'))

            self.file.saved_to = self._rename_if_exists(self.file.saved_to)

        else:  # normal path
            self.file.saved_to = str(Path(self.destination) / (valid_filename + file_extension))
    ...
    def try_move_file(self) -> bool:
        ...
        path = self.file.old_file.saved_to
        if not os.path.exists(path):
            return False

        logging.debug('T%s - Moving old file "%s" to new target location', self.thread_id, path)
        try:
            shutil.move(path, self.file.saved_to)
            self.file.time_stamp = int(time.time())
            self.success = True
            return True
        except FileExistsError:
            ...
            try:
                ...
                logging.warning('T%s - Moving the old file %s failed!  Error: %s', self.thread_id, path, e)

            self.fs_lock.release()
        except Exception as e:
            logging.warning('T%s - Moving the old file %s failed unexpectedly!  Error: %s', self.thread_id, path, e)

        return False

    def try_delete_file(self) -> bool:
        """
        It will try to delete the old file.
        If it worked it returns True.
        """
        ...
        logging.debug('T%s - Delete old file "%s"', self.thread_id, path)
        ...
        try:
            ...
            logging.warning('T%s - Delete the old file %s failed unexpectedly!  Error: %s',
                            self.thread_id, path, e)
    ...
    def download(self, thread_id: int):
        ...
        try:
            ...
            if self.options.get('download_modified_file', True):
                # if file was modified try rename the old file,
                # before create new one
                if self.file.modified:
                    self.try_rename_old_file()

            if self.options.get('download_delete_file', True):
                # Try to delete the old file if it still exists
                if self.file.deleted:
                    if self.try_delete_file():
                        return self.success

            # create a empty destination file
            self.set_path()

            if self.options.get('download_move_file', True):
    ...
    def urlretrieve(self, url: str, filename: str, context: ssl.SSLContext, reporthook=None):
        """
        original source:
