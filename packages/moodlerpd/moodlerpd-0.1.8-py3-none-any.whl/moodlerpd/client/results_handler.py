...
class ResultsHandler:
    ...
    @staticmethod
    def should_download_course(course: Course, category: bool,
                               download_course_ids: [int], dont_download_course_ids: [int]) -> bool:
        """
        Checks if a course is in White-list and not in Blacklist
        """
        in_blacklist = course.id in dont_download_course_ids
        in_whitelist = course.id in download_course_ids or len(download_course_ids) == 0
        check_category = isinstance(course.category_id, int) if category else True

        return check_category and in_whitelist and not in_blacklist
