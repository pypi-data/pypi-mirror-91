...
--q_type, 量化类型(0-对称, 1-非对称); --q_lmicronetl, 权重量化级别(0-通道级, 1-层级); --bn_fuse, 量化中bn融合标志(0-不融合, 1-融合); --weight_observer, weight_observer选择(0-MinMaxObserver, 1-MovingAverageMinMaxObserver)

- (默认)对称、(权重)通道级量化, bn不融合, weight_observer-MinMaxObserver

```bash
python main.py --q_type 0 --q_lmicronetl 0 --bn_fuse 0 --weight_observer 0 --gpu_id 0
```

- 对称、(权重)通道级量化, bn不融合, weight_observer-MovingAverageMinMaxObserver

```bash
python main.py --q_type 0 --q_lmicronetl 0 --bn_fuse 0 --weight_observer 1 --gpu_id 0
```

- 对称、(权重)层级量化, bn不融合

```bash
python main.py --q_type 0 --q_lmicronetl 1 --bn_fuse 0 --gpu_id 0
```

- 非对称、(权重)通道级量化, bn不融合

```bash
python main.py --q_type 1 --q_lmicronetl 0 --bn_fuse 0 --gpu_id 0
```

- 非对称、(权重)层级量化, bn不融合

```bash
python main.py --q_type 1 --q_lmicronetl 1 --bn_fuse 0 --gpu_id 0
```

- 对称、(权重)通道级量化, bn融合

```bash
python main.py --q_type 0 --q_lmicronetl 0 --bn_fuse 1 --gpu_id 0
```

- 对称、(权重)层级量化, bn融合

```bash
python main.py --q_type 0 --q_lmicronetl 1 --bn_fuse 1 --gpu_id 0
```

- 非对称、(权重)通道级量化, bn融合

```bash
python main.py --q_type 1 --q_lmicronetl 0 --bn_fuse 1 --gpu_id 0
```

- 非对称、(权重)层级量化, bn融合

```bash
python main.py --q_type 1 --q_lmicronetl 1 --bn_fuse 1 --gpu_id 0
```
...
#from micronet.compression.quantization.wbwtab.quantize import QuantConv2d
#from micronet.compression.quantization.wqaq.dorefa.quantize import QuantConv2d, QuantLinear
from micronet.compression.quantization.wqaq.iao.quantize import QuantConv2d, QuantLinear # quantize is quant_module, QuantConv2d and QuantLinear are quant_op
