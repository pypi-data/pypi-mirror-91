...
def _build_upload_script_msg(fileName: str = None, content: bytes = None, cmdId: int = 1):
    ...
    # message
    message: _Message = msg_utils.build_request_msg(cmdId, 0, request)
    return message


# 上传python脚本到机器人
def upload_script(cmdId: int, robot_id: str, fileName: str = None, content: bytes = None):
    ...
    if device is None:
        ...
        if device is None:
            ...
            _found_devices[robot_id] = device
    # 触发
    return asyncio.run(_send_msg0(_build_upload_script_msg(fileName, content, pyCmdId.value), device))
