...
def generic_error_handling(exctype, exc):
    global current_error_count
    current_error_count = current_error_count + 1
    msg = "*** Failure running statement"
    print(msg)
    print(exc)
...
def log_on(user=None, password=None, account=None, database=None,warehouse=None,role=None, login_timeout = 10):
        ...
        if (role is None):
            role = get_from_args_or_environment(5, "SNOW_ROLE", args)
        try:
            ...
            exctype, exp, traceback = sys.exc_info()
            generic_error_handling(exctype, exp)
            error_code = 333
...
def execute_sql_statement(sql_string, con, using=None):
    ...
    try:
        ...
        # we will handle only sql error
        # any other errors will be handle by the exception hook
        _handle_sql_error(e)
    finally:
...
def _handle_sql_error(e):
    global error_code, error_level, current_error_count
    current_error_count = current_error_count + 1
    error_code = e.errno
