...
class FOFDataManager(metaclass=Singleton):
    ...
    @staticmethod
    def _do_calc_v_net_value(nav: Union[pd.Series, float], init_water_line: pd.Series, incentive_fee_ratio: Union[pd.Series, float], decimals: Union[pd.Series, float]) -> pd.Series:
        # 盈利
    ...
    def get_investor_return(fof_id: str, *, investor_id_list: Tuple[str] = (), start_date: Optional[datetime.date] = None, end_date: Optional[datetime.date] = None) -> Optional[pd.DataFrame]:
        ...
        for row in fof_scale_info.itertuples(index=False):
            ...
            try:
                ...
                init_water_line = 1
            investor_mvs[row.investor_id].append({'start_date': actual_start_date, 'share': row.unit_total, 'init_mv': init_mv, 'init_water_line': init_water_line})
        ...
        for investor_id, datas in investor_mvs.items():
            ...
            avg_v_nav = latest_mv / datas.share.sum()
            total_rr = latest_mv / datas.init_mv.sum() - 1
            investor_returns[investor_id] = {'v_nav': avg_v_nav, 'total_rr': total_rr}
