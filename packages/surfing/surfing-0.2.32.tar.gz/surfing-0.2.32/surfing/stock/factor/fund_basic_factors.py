...
class IndexRetWeekly(Factor): # based on IndexRetDaily
    ...
    def calc(self):
        ...
        self._factor = self.data_resample_weekly_ret(self._factor).dropna(axis=0, how='all')
...
class IndexRetMonthly(Factor): # based on IndexRetDaily
    ...
    def calc(self):
        self._factor = IndexRetDaily().get()
        self._factor = self.data_resample_monthly_ret(self._factor).dropna(axis=0, how='all')
...
class FundNavWeeklyModify(Factor): # based on FundNavDailyModify
    ...
    def calc(self):
        ...
        self._factor = self.data_resample_weekly_nav(self._factor).dropna(axis=0, how='all')
...
class FundNavMonthlyModify(Factor): # based on FundNavDailyModify
    ...
    def calc(self):
        self._factor = FundNavDailyModify().get()
        self._factor = self.data_resample_monthly_nav(self._factor).dropna(axis=0, how='all')
