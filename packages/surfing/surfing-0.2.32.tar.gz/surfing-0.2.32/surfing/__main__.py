...
class SurfingEntrance(object):
    ...
    def check_refinancing(self, yesterday=None, wechat_bot=None):
        ...
        # TODO: 目前key是hard code的
        key = '高瓴'
        df = df[df.issue_object.str.contains(key)]
        if df.empty:
            return
        stock_info = RawDataApi().get_em_stock_info(stock_list=df.stock_id.to_list())
    ...
    def check_refinancing_impl(self, yesterday=None, wechat_bot=None):
        ...
        if stock_info is None:
            return
        stock_price = RawDataApi().get_em_stock_price(start_date=yesterday, end_date=yesterday, stock_list=df.stock_id.to_list(), columns=['close'])
        if stock_price is None:
            return
        stock_info = stock_info.set_index('stock_id')[['name']].join([stock_price.drop(columns='datetime').set_index('stock_id'), df.set_index('stock_id').issue_price], how='outer')
        if stock_info.empty:
            return
