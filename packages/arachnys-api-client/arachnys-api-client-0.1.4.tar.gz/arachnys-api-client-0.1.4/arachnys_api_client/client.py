from typing import Any, Dict, KeysView, List, NamedTuple, Optional, Tuple
...
class Attribute(Dict[str, Any]):
    ...
    def simple_value(self) -> Any:
        if "local_string" in self["value"]:
            return self["value"]["local_string"]["values"][self["value"]["local_string"]["official_locale"]]
        else:
...
class AttributeMap:
    ...
    def __repr__(self):
        return dict(**self._attributes).__repr__()
...
class APIClient:
    ...
    def _make_authenticated_call(
        ...
        while elapsed_retry_time < max_retry_time:
            ...
            if resp.status_code in [403, 401] and retry_on_token_expired is True:
