...
from .sea_print import print_results
...
def main(debug=False):
    ...
    if not validate(args.parameter_file, verbose=True):
        sys.exit(2)
    ...
    t = print_results(events, args.parameter_file, params,
                      orig_stations, used_stations)
    print(t)
    outfname = Path(args.parameter_file).name.split('.')[0] + '.rst'
    with open(outfname, 'w') as f:
        f.write(t)
    htmlfname = Path(args.parameter_file).name.split('.')[0] + '.html'

    docutils.core.publish_file(source_path=outfname,
                               destination_path=htmlfname, writer_name="html")
    os.remove(outfname)
    # Make a map
    if not args.no_map:
...
def read_param_file(parameter_file):
    ...
    p = yaml.load(stream, Loader=yaml.SafeLoader)
    stream.close()
    event_list = p['events']
