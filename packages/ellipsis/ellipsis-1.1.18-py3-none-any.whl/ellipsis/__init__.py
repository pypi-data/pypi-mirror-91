...
def geometryGet(mapId, layer, geometryIds = None, historyFilter = None, filters = None, xMin = None, xMax = None, yMin=None, yMax=None, wait = 0, showLoadingBar = False, deleted = False, token = None):
...
def geometryAdd(mapId, layer, features, token):
    ...
    firstTime = metadata(mapId = mapId, token = token)['geometryLayers']
    ...
    if firstTime:
        print('no properties for this layer know, adding them automatically')
        ...
        for c in columns:
            ...
            addProperty(mapId = mapId, layer =layer, propertyName = c, propertyType = propertyType, token = token)
...
def addProperty(mapId, layer, propertyName, propertyType, token, private=False, required=False):
...
def newMap(name,  token, bands = [{'name':'red', 'number':1}, {'name':'green', 'number':2}, {'name':'blue', 'number':3}], visualizations= [{'name':'rgb', 'a':0,'b':0, 'c':1}], model = None, fitToRaster = True, boundary = None, resolutionFromRaster = True, useMask=False, dataType = 'float32' , dryRun = True):
    if resolutionFromRaster:
        for i in np.arange(len(bands)):
            bands[i]['resolution'] = 0.008
            
    if (not fitToRaster) and str(type(boundary))==str(type(None)):
        raise ValueError('if fitToRaster false, you must provide a boundary')
    ...
    body = {'name':name, 'dataSource':bands, 'useMask':useMask , 'captures': [], 'visualizations':visualizations, 'measurements':measurements, 'active':False, 'dataType':dataType, 'fitToRaster':fitToRaster, 'resolutionFromRaster':resolutionFromRaster, 'dryRun':dryRun }
...
def projectProperties(mapId, properties, token):
