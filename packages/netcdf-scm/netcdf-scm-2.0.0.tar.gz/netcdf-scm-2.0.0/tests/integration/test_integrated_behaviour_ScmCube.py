...
class _ScmCubeIntegrationTester(object):
    ...
    def test_get_scm_timeseries_weights_one_invalid(
        self, test_cube,
    ):
        # no error but only valid keys returned
        res = test_cube.get_scm_timeseries_weights(regions=["World", "junk"])
        assert list(res.keys()) == ["World"]

    def test_get_scm_timeseries_weights_all_valid(
        self, test_cube,
    ):
        # no error but empty dictionary returned
        res = test_cube.get_scm_timeseries_weights(regions=["junk"])
        assert res == {}
...
class TestCMIP6OutputCube(_CMIPCubeIntegrationTester):
    ...
    def test_load_hfds_data(self, test_cube, test_cmip6_output_hfds_files):
        ...
        error_msg = re.escape("No valid weights for regions: ['World|Land']")
