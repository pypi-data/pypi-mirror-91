...
class DeployError(MpaasError):
    """
    Raised when there is an error in the kubectl deploying process
    """

    def __init__(self, resource, kubectl_message=None):
        super().__init__()
        self.message = "%s could not be deployed by kubectl." % resource
        self.kubectl_message=kubectl_message

    def print_kube_message(self):
        if self.kubectl_message:
            print(self.kubectl_message)
            return True
        return False
