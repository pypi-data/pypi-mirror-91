...
### Rights import from JSON

This can be made using CSVFilteringCadastre class.

First, CSVFilteringCadastre must be initialized with a connection to Orion :

```python
csv_filtering_cadastre = CSVFilteringCadastre(orion)
```

Then, a CSV file must be generated form the json file :

```python
csv_filtering_cadastre.generate("my_csv.csv", "my_json.json")
#"my_csv.csv" is the csv file that you will generate
#"my_json.json" is the json file containing data
```

Finally, the rights can be imported :

```python
csv_filtering_cadastre.read_and_apply("my_csv.csv")
#"my_csv.csv" is the csv file generated previously
```
