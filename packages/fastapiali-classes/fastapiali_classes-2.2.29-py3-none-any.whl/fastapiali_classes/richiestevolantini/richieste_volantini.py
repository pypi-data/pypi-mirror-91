    ...
    def find(cls, dt_inizio_creazione: Optional[Union[datetime.datetime, str]] = None,
        ...
        try:
            ...
            if dt_inizio_creazione:
                inizio_creazione = datetime.datetime.strptime(dt_inizio_creazione.split("T")[0], "%Y-%m-%d") \
            ...
            if dt_fine_creazione:
                fine_creazione = datetime.datetime.strptime(dt_fine_creazione.split("T")[0], "%Y-%m-%d") \
                    if type(dt_fine_creazione) == str else dt_fine_creazione
                rcds_ric = rcds_ric.filter(Richieste.dt_creazione <= datetime.datetime.combine(
                    fine_creazione, datetime.datetime.max.time()))
            if dt_inizio_ric:
                inizio_ric = datetime.datetime.strptime(dt_inizio_ric.split("T")[0], "%Y-%m-%d").date() \
                    if type(dt_inizio_ric) == str else dt_inizio_ric
                rcds_ric = rcds_ric.filter(Richieste.dt_inizio >= datetime.datetime.strftime(inizio_ric, "%Y-%M-%d"))
            if dt_fine_ric:
                fine_ric = datetime.datetime.strptime(dt_fine_ric.split("T")[0], "%Y-%m-%d").date() \
                    if type(dt_fine_ric) == str else dt_fine_ric
                rcds_ric = rcds_ric.filter(Richieste.dt_fine <= datetime.datetime.strftime(fine_ric, "%Y-%M-%d"))
