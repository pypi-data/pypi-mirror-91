    ...
    def find(cls, dt_inizio_creazione: Optional[Union[datetime.datetime, str]] = None,
        ...
        try:
            ...
            if dt_inizio_ric:
                ...
                rcds_ric = rcds_ric.filter(Richieste.dt_inizio >= datetime.datetime.strftime(datetime.datetime.combine(
                    inizio_ric, datetime.datetime.min.time()), "%Y-%M-%dT%H:%M:%sZ"))
            if dt_fine_ric:
                fine_ric = datetime.datetime.strptime(dt_fine_ric, "%Y-%M-%d") \
                    if type(dt_fine_ric) == str else dt_fine_ric
                rcds_ric = rcds_ric.filter(Richieste.dt_fine <= datetime.datetime.strftime(datetime.datetime.combine(
                    fine_ric, datetime.datetime.max.time()), "%Y-%M-%dT%H:%M:%sZ"))
