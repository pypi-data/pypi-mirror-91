...
def query_weather(city_name):
    ...
    print("\n%s\n%s**\n%s\n%s\u00b0C \u2248 %s\u00b0F\nPrecipitation: %s\nHumidity: %s\nWind: %s \u2248 %s\n%s" % 
	    (city.text.strip(), time.text.strip(),condition.text.strip(), tempC.text.strip(), tempF.text.strip(),precipitation.text.strip(),
            humidity.text.strip(),
	        wind_kmh.text.strip(), wind_mph.text.strip(), "**This is the last weather update time."))
...
if __name__== "__main__":
    ...
    import sys
    if(len(sys.argv)<=1):
        print("Usage: googleweather <city name>")
    else:
