...
def latestVersion(force=False):
    ...
    if url_info.headers['Content-Type'] != 'application/octet-stream':
        ...
        if url_info.headers['Content-Length'] < 5000000 or datetime.date.today().weekday() == 3 or force:
            if url_info.headers['Content-Length'] > 5000000:
                log.debug('file is {}Mb long.'.format(url_info.headers['Content-Length'] / 1000000))
