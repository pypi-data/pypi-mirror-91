...
{
    if (*distance_remaining < 0 && *T >= jump_time)
        return 1;
    ...
    _Bool forward = forward_time > 0 && can_reach_target_forwards(forward_time, jump_time, boost_accel, distance_remaining, &true_car_speed, &me->boost, max_speed_reduction);
    true_car_speed *= -1;
