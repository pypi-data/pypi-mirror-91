...
class MiniBertForMLMAndRegression(nn.Module):
    def __init__(self, configuration):
        ...
        self.reg_l1_activation_fun = parse_activation_function(
            configuration.reg_first_layer_activation_fun)

        self.mlm_l2_activation_fun = parse_activation_function(
            configuration.mlm_second_layer_activation_fun)
        self.reg_l2_activation_fun = parse_activation_function(
            configuration.reg_second_layer_activation_fun)
    ...
    def forward_reg(self, input):
        x = self.minibert(input)
        x = self.reg_l1(x)
        x = self.reg_l1_activation_fun(x)
        x = self.reg_l2(x)
