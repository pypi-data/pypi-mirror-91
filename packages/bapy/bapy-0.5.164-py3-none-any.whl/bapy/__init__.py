...
class Sem(AsdictLarge):
    atlas: int = 499
    atlas_new: int = round(atlas / 20)
    db: int = 500
    io: int = 500
    max: int = 5000
    nmap: int = 500
    nmap_new: int = round(nmap / 20)
    os: int = 300
    os_new: int = round(os / 20)
    ping: int = 400
    ping_new: int = round(ping / 20)
    socket: int = 500
    ssh: int = 300
    ssh_new: int = round(ssh / 20)
    _factor: int = 1
...
class MongoBase:
    ...
    def db_default(self, a: bool = False) -> Any:
    ...
    def nap(self, mongo: bool = True):
        if mongo:
            time.sleep(self.naps.mongo)
        time.sleep(round(random.uniform(self.naps.os[0], self.naps.os[1]), 2))

    async def anap(self, mongo: bool = True):
        if mongo:
...
class Mongo(MongoBase):
    ...
    def col(self, a: bool = False) -> Any:
        return self.db(a).get_collection(self.col_name(a))

    def col_default(self, a: bool = False) -> Any:
    ...
    def col_find(self, find: dict = None) -> int:
        while True:
            try:
