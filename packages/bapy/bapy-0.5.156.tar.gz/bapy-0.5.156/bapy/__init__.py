...
class Env(Asdict):
    ...
    def sem(self):
        return Sem(*[self.file.int(upper_prefix(k, prefix=self.prefix), v)
                     for k, v in Sem(_factor=self.sem_factor).defaults().items()
                     if k != 'value'] + [self.sem_factor])
...
class Frame:
    ...
    def __init__(self, index: int = index, ctx: int = ctx):
        ...
        if back := getattr(frame, 'f_back', None):
            ...
            self.back_package: str = self.globals.get('__package__', None)
...
class Logr(Logger):
    ...
    @classmethod
    def prepare_msg(cls, sync, msg, *args) -> str:
        # def frame(i):
        #     try:
...
class Mongo(MongoBase):
    ...
    def unique(self, field: str = '_id') -> list:
        args = Frame.args()
        path.log.debug('Start', args)
        while True:
            try:
                rv = self.col.distinct(field)
                path.log.info('End', args)
                return rv
            except self.mongo_exceptions as exception:
                path.log.awarning(f'Waiting for connection: {repr(exception)}')
                time.sleep(self.naps.mongo)
                continue
        path.log.info('End', args)
