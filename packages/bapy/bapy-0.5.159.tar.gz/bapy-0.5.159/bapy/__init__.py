    ...
    def __init__(self, index: int = index, ctx: int = ctx):
        ...
        if back_f_back := getattr(back, 'f_back', None):
            if 'importlib._bootstrap' in str(back_f_back):
                if frame_back_back := getattr(back_f_back, 'f_back', None) is None:
                    raise BapyFrameBackBack(str(back_f_back))
                else:
                    frame = frame_back_back
            self.back_back = getattr(frame, 'f_back', None)
