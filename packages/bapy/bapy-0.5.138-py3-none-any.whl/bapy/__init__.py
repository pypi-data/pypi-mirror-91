...
class GitHub:
    ...
    def __post_init__(self):
        ...
        with self.lock:
            if self.log:
                self.log.debug(self.__class__.__name__, f'{self.lock.locked()=}')
            self.init_clone()
    ...
    def add(self, force: Bool = False, write: Bool = True):
        ...
        for file in self.repo.untracked_files:
            ...
            rv.append(added[0][3])
        if self.log:
            self.log.debug(f'{self.lock.locked()=}', rv)
        return rv
    ...
    def commit_cmd(self, message: Text = None) -> None:
        ...
        if self.log:
            self.log.debug(f'{self.lock.locked()=}', commit)
        return commit
    ...
    def commit_cli(self, message, ctx, extra_args=None):
        ...
        try:
            ...
            if self.log:
                self.log.debug(f'{self.lock.locked()=}', err_msg)
            raise exc
        finally:
            os.unlink(f.name)
    ...
    def init_clone(self):
        ...
        try:
            ...
            if self.log:
                self.log.error(f'{self.lock.locked()=}', exception)
    ...
    def push(self, remote: Text = 'origin'):
        ...
        if self.log:
            self.log.debug(f'{self.lock.locked()=}', f'{rv[0].summary=}', f'{rv[0].flags=}')
...
class Logr(Logger):
    ...
    def prepare_msg(cls, sync, msg, *args) -> Text:
        try:
            ...
            frame = f.back.f_back
            try:
                file_async = Path(frame.f_globals.get('__file__'))
                file_async_name = file_async.name
            except AttributeError:
                sync = False
                file_async = file_async_name = None
            file, func, line = (file_async, frame.f_code.co_name, frame.f_lineno,) \
                if sync and file_async_name != 'events.py' else (f.back_file, f.back_function, f.back_lineno,)
            func_line = f'({file.name} | {func}: {line}) '
        except IndexError:
            func_line = str()
        rv = ', '.join(str(item) for item in (msg, *args,))
        rv = func_line + rv if func_line else rv
        return rv
    ...
    async def adebug(self, msg, *args, **kwargs):
        if self.isEnabledFor(logging.DEBUG):
            await executor(self._log, logging.DEBUG, self.prepare_msg(True, msg, *args), (), **kwargs)

    async def ainfo(self, msg, *args, **kwargs):
        if self.isEnabledFor(logging.INFO):
            await executor(self._log, logging.INFO, self.prepare_msg(True, msg, *args), (), **kwargs)

    async def awarning(self, msg, *args, **kwargs):
        if self.isEnabledFor(logging.WARNING):
            await executor(self._log, logging.WARNING, self.prepare_msg(True, msg, *args), (), **kwargs)

    async def aerror(self, msg, *args, **kwargs):
        if self.isEnabledFor(logging.ERROR):
            await executor(self._log, logging.ERROR, self.prepare_msg(True, msg, *args), (), **kwargs)

    async def acritical(self, msg, *args, **kwargs):
        if self.isEnabledFor(logging.CRITICAL):
            await executor(self._log, logging.CRITICAL, self.prepare_msg(True, msg, *args), (), **kwargs)

    async def aexception(self, msg, *args, exc_info=True, **kwargs):
        await self.aerror(self.prepare_msg(True, msg, *args), (), exc_info=exc_info, **kwargs)
