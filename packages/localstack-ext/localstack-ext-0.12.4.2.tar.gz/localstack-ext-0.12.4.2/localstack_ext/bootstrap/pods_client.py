...
class CloudPodManager(CXWcp):
 BACKEND='_none_'
 CONFIG_FILE='.localstack.yml'
 def __init__(self,pod_name=CXWci,config=CXWci):
  self.pod_name=pod_name
  self.config=config
 def push(self):
  raise CXWct('Not implemented')
 def pull(self):
  raise CXWct('Not implemented')
 ...
 def get(cls,pod_name,config=CXWci):
  ...
  raise CXWct('Unable to find Cloud Pod manager implementation type "%s"'%backend)
 @CXWcL
 def load_config(cls,safe=CXWcN):
  try:
   return yaml.load(to_str(load_file(cls.CONFIG_FILE)))
  except CXWct:
   if safe:
    return{}
   raise CXWct('Unable to find and parse config file "%s"'%cls.CONFIG_FILE)
 @CXWcL
 def data_dir(cls):
  ...
  if not config.DATA_DIR:
   raise CXWct('Working with local cloud pods requires $DATA_DIR configuration')
...
class CloudPodManagerGit(CloudPodManager):
 ...
 def push(self):
  ...
  is_empty_repo=b'HEAD' not in repo or repo.refs.allkeys()==CXWcz([b'HEAD'])
  if is_empty_repo:
  ...
  if branch not in repo:
   porcelain.branch_create(repo,branch,force=CXWcu)
 ...
 def pull(self):
  ...
  if remote_ref not in remote_refs:
   raise CXWct('Unable to find branch "%s" in remote git repo'%branch)
  remote_location=self.pod_config.get('url')
  self.switch_branch(branch)
 ...
 def client(self):
  ...
  return client,path
 def local_repo(self):
  self.clone_dir=CXWcU(self,'clone_dir',CXWci)
...
def store_pod_metadata(pod_name,metadata):
 ...
 if response.status_code>=400:
  raise CXWct('Unable to store pod metadata in API (code %s): %s'%(response.status_code,content))
...
def list_pods(args):
 ...
 content=response.content
 if response.status_code>=400:
  raise CXWct('Unable to fetch list of pods from API (code %s): %s'%(response.status_code,content))
 ...
 local_pods=CloudPodManager.load_config(safe=CXWcu).get('pods',{})
 local_pods=[{'pod_name':k,'state':'Local Only',**v}for k,v in local_pods.items()]
 existing_names=CXWcz([pod['pod_name']for pod in result])
 result.extend([pod for pod in local_pods if pod['pod_name']not in existing_names])
 return result
def _pod_config(pod_name,config=CXWci):
 config=config or CloudPodManager.load_config()
 pod_config=config.get('pods',{}).get(pod_name)
 if not pod_config:
  raise CXWct('Unable to find config for pod named "%s"'%pod_name)
 return pod_config
