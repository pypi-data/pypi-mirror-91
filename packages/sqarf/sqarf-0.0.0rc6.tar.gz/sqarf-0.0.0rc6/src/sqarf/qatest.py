...
class QATest(object):
    ...
    def __init__(self):
        ...
        # will be filled in _do_test(), fix(), but also
    ...
    def run(self, base_context):
        ...
        # run this test
        self.log.debug("=> calling self._do_test(context)")
        try:
            passed, why = self._do_test(self._context)
        except Exception as err:
            self.log.debug("self._do_test() raised", err)
            trace = traceback.format_exc()
            ...
            self.log.debug("self._do_test() returned", (passed, why))
            if passed:
