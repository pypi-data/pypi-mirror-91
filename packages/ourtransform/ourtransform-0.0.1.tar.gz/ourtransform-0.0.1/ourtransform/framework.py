...
class Changeable(Event):
    ...
    def __verify_type__(element: Element):
        if not issubclass(type(element), Element):
            raise Exception("Input element not of type Element")
...
class Selector(object):
    ...
    """
    def __init__(self, chains: List[Chain], logger = None):
        self.chains = {}
        self.logger = logger
        for chain in chains:
...
class Result(object):
    ...
    """
    def __init__(self, elements: List[Element], notices: List[Notice]):
...
class Process(object):
    ...
    """
    def __init__(self, selector: Selector, notice_level: Level = Level.ERROR, meta: Meta = None):
    ...
    def _run(process, elements: List[Element]) -> Result:
        ...
        for element in elements:
            ...
            except ChainNotFoundException as cnf:
                element.notices.append(
                    Notice(
                        msg=f"{cnf}",
                        level=process.notice_level,
                    )
                )
