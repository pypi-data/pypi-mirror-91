...
class EzvizCamera(object):
    ...
    def load(self):
        ...
        if self._device['supportExt']['support_sensibility_adjust']:
            self._sleep_switch_set = False
            #Check if camera auto sleep is enabled.
            for switch in self._device['deviceSwitchStatuses']:
                if switch['type'] == DeviceSwitchType.AUTO_SLEEP.value:
                    if switch['enable'] == True:
                        self._sleep_switch_set = True
                    break

            if self._sleep_switch_set != True:
            ...
            if self._sleep_switch_set == True:
                self._detection_sensibility = "Sleep"
    ...
    def status(self):
        ...
        return {
            ...
            'sleep': self.get_switch(DeviceSwitchType.SLEEP) or self._sleep_switch_set,
